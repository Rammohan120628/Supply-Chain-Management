...
export interface TableTypeDense {
  ...
  status?: string;  
  supplierName?: string;        // deliveryMode / supplierName
  teams?: string;               // itemName
  statuscolor?: string;
  budget?: string;              // grandTotal / quantity
  gross?: string;
  net?: string;
}
...
const defaultColumns2Full = (formatter: ReturnType<typeof useEntityFormatter>) => [
  ...
  columnHelper.accessor('pname', {
    ...
    cell: (info) => {
      ...
      return (
        <span className="text-[11px] font-medium text-gray-700 dark:text-white">
          {formattedDate}
        </span>
      );
    },
...
const LocReqTable = ({ onBack }: { onBack?: () => void }) => {
    ...
    <div className="w-[1050px]">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md p-2 sm:p-4">
        <div className="w-full">
          ...
          {/* Tabs */}
          <Tabs
            aria-label="Location request tabs"
            variant="underline"
