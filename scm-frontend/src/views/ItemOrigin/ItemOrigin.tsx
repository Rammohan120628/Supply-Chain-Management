...
// ---------- Confirm Modal (only for status changes and create) ----------
interface ConfirmModalProps {
  isOpen: boolean;
  confirmType: 'status' | 'add'; // we only need status and add here
  onConfirm: () => void;
  onCancel: () => void;
  targetName?: string;
}
...
const ConfirmModal: React.FC<ConfirmModalProps> = ({
  ...
  const title = isStatus ? 'Confirm Status Change' : 'Confirm Save';
  const message = isStatus
    ? `Are you sure you want to change the status of "${targetName || 'this item origin'}"?`
    : `Are you sure you want to save this new item origin "${targetName || ''}"?`;
  const iconColor = isStatus ? 'text-red-600' : 'text-green-600';
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-300 scale-95 hover:scale-100 border border-white/20 dark:border-gray-700/50 ring-1 ring-gray-900/5 dark:ring-white/10 animate-scaleIn">
        <div className="p-6 sm:p-8">
          <div className="text-center mb-6">
            <div className={`mx-auto w-16 h-16 rounded-full flex items-center justify-center mb-4 ring-4 shadow-lg bg-gradient-to-br from-${isStatus ? 'red' : 'green'}-100 to-${isStatus ? 'red' : 'green'}-200 dark:from-${isStatus ? 'red' : 'green'}-900/30 dark:to-${isStatus ? 'red' : 'green'}-800/30 ring-${isStatus ? 'red' : 'green'}-200/50 dark:ring-${isStatus ? 'red' : 'green'}-900/30`}>
              <Package className={`w-8 h-8 animate-pulse ${iconColor} dark:${iconColor.replace('text-', 'dark:text-')}`} />
            </div>
...
const ItemOriginList: React.FC<ItemOriginTableProps> = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<ItemOriginData | null>(null);
  ...
  const [newItemOriginName, setNewItemOriginName] = useState('');
  ...
  // Confirm modal state (for status change and create)
  const [showConfirm, setShowConfirm] = useState(false);
  const [confirmType, setConfirmType] = useState<'status' | 'add' | null>(null);
  ...
  const LIST_API_URL = `${BASE_URL}/listMstItemOrigin`;
  const VIEW_API_URL = `${BASE_URL}/viewMstItemOrigin`;
  const MODIFY_API_URL = `${BASE_URL}/modifyMstItemOrigin`;
  const STATUS_UPDATE_API_URL = `${BASE_URL}/mstItemOriginStatusUpdate`;
  const SAVE_API_URL = `${BASE_URL}/saveMstItemOrigin`;
  ...
  const openEdit = async (item: ItemOriginData) => {
    ...
    try {
      ...
      setEditOriginName(fullItem.itemOriginName || item.itemOriginName);
      ...
      setShowSessionExpired(true);
      console.error('Error fetching item origin details:', error);
      showToast('Error fetching item origin details.', 'error');
      setEditOriginName(item.itemOriginName);
  ...
  const handleEditSave = async () => {
    const trimmed = editOriginName.trim();
    if (!trimmed) {
      showToast('Item Origin Name is required.', 'error');
      setErrors({ itemOriginName: 'Item Origin Name is required.' });
      return;
    }
    if (trimmed.length < 4 || trimmed.length > 15) {
      showToast('Item Origin Name must be between 4 and 15 characters.', 'error');
      setErrors({ itemOriginName: 'Must be 4-15 characters.' });
      return;
    }
    ...
    const updateBody = {
      itemOriginPk: selectedItem.id,
      itemOriginName: trimmed,
      status: statusCode,
    ...
    try {
      ...
      setData((prevData) =>
        prevData.map((item) =>
          item.id === selectedItem.id
            ? { ...item, itemOriginName: trimmed, status: selectedItem.status, statusColor: newStatusColor }
            : item
        )
      );
      ...
      setShowSessionExpired(true);
      console.error('Error updating item origin:', error);
      showToast('Error updating item origin.', 'error');
    } finally {
      NProgress.done();
    }
  };
  ...
  const validateCreateFields = (): boolean => {
    const trimmed = newItemOriginName.trim();
    if (!trimmed) {
      showToast('Item Origin Name is required.', 'error');
      setNewErrors({ itemOriginName: 'Item Origin Name is required.' });
      return false;
    }
    if (trimmed.length < 4 || trimmed.length > 15) {
      showToast('Item Origin Name must be between 4 and 15 characters.', 'error');
      setNewErrors({ itemOriginName: 'Must be 4-15 characters.' });
      return false;
    }
    return true;
  };

  // ----- Create Save with Confirm -----
  const handleCreateClick = () => {
    if (!validateCreateFields()) return;
    setTargetItemName(newItemOriginName.trim());
    setConfirmType('add');
    setConfirmAction(() => performCreate);
    setShowConfirm(true);
  };
    ...
    try {
      ...
      if (data.success === false) {
        if (data.message?.toLowerCase().includes('duplicate')) {
          showToast('Item origin name already exists', 'error');
        } else {
          showToast(data.message || 'Error saving item origin.', 'error');
        }
        return;
      }

      showToast('Item origin saved successfully.', 'success');
      closeCreateModal();
      fetchData(); // refresh the list
    } catch (error) {
      console.error('Error saving item origin:', error);
      showToast(
        error instanceof Error ? error.message : 'Failed to save item origin.',
        'error',
      );
    } finally {
      NProgress.done();
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const mappedData = res.data.map((item: any) => ({
        ...
        itemOriginName: item.itemOriginName,
      ...
      setShowSessionExpired(true);
      console.error('Error fetching item origins:', error);
      showToast('Error fetching item origins.', 'error');
    } finally {
      setIsLoading(false);
      NProgress.done();
    }
  ...
  const defaultColumns = [
    ...
    columnHelper.accessor('itemOriginName', {
      header: ({ column }) => (
        ...
        >
          <span className="font-medium text-white text-[10px] uppercase">Item Origin Name</span>
          {column.getCanSort() && (
        ...
        return (
          <label className="inline-flex items-center cursor-pointer">
            <input
              ...
              checked={isActive}
              onChange={() => handleStatusChange(row.id, status, row.itemOriginName)}
              disabled={isToggling}
            />
            <div
              className={`
    ...
    columnHelper.display({
      ...
      cell: ({ row }) => (
        <button
          ...
          title="Edit item origin"
          aria-label={`Edit ${row.original.itemOriginName}`}
        >
          <SquarePen size={16} />
        </button>
      ),
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-2 md:p-3">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm mb-4 mt-2 p-4">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
                ...
                content={
                  <div className="text-xs max-w-xs">
                    ...
                    <ol className="list-decimal list-inside space-y-1">
                      <li>Click the "+" button to add a new item origin</li>
                      <li>Fill in the name (min 4, max 15 characters)</li>
                      <li>Use the modify icon to edit an existing origin</li>
            ...
            <div className="flex gap-2">
              <Tooltip content="Add new item origin" placement="bottom">
                <Button
                  color="blue"
                  size="xs"
                      ...
                      <td colSpan={columns.length} className="px-1.5 py-8 text-center">
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-1">
                            {debouncedSearchTerm ? 'No matching records found' : 'No item origins yet'}
                          </h4>
                          <p className="text-gray-500 dark:text-gray-400 text-xs mb-4 text-center max-w-sm">
                            {debouncedSearchTerm
                              ? 'Try a different search term or clear the filter.'
                              : 'Get started by creating your first item origin.'}
                          </p>
                          {!debouncedSearchTerm && (
                            <Button
                              color="blue"
                            ...
                            >
                              <HiPlus className="w-4 h-4" /> Add Item Origin
                            </Button>
                          )}
                        </div>
                      </td>
        ...
        <Modal show={showCreateModal} onClose={closeCreateModal} size="md" dismissible>
          <ModalHeader className="rounded-lg border-b bg-white dark:bg-gray-800 text-white p-4">
            <div className="flex items-center gap-2">
              <HiPlus className="w-4 h-4" />
              <span className="text-sm font-bold">Add Item Origin</span>
            </div>
          </ModalHeader>
          <ModalBody className="p-4 bg-white dark:bg-gray-800">
            <div className="space-y-4">
              <FloatingInput
                id="newItemOriginName"
                name="itemOriginName"
                value={newItemOriginName}
                onChange={(e) => {
                  setNewItemOriginName(e.target.value);
                  setNewErrors(prev => { const newErrors = { ...prev }; delete newErrors.itemOriginName; return newErrors; });
                }}
                label="Item Origin Name"
                required
                error={newErrors.itemOriginName}
                maxLength={15}
              />
            </div>
          </ModalBody>
        ...
        <Modal show={showEditModal} onClose={closeEdit} size="md" dismissible>
          <ModalHeader className="rounded-lg border-b bg-white dark:bg-gray-800 text-white p-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold">Edit Item Origin</span>
            </div>
          </ModalHeader>
          <ModalBody className="p-4 bg-white dark:bg-gray-800">
            <div className="space-y-4">
              ...
              <div className="flex justify-end mb-2">
                <button
                  onClick={() => {
                    if (selectedItem) {
                      handleStatusChange(selectedItem.id, selectedItem.status, selectedItem.itemOriginName);
              ...
              <FloatingInput
                ...
                onChange={(e) => {
                  setEditOriginName(e.target.value);
                  setErrors(prev => { const newErrors = { ...prev }; delete newErrors.itemOriginName; return newErrors; });
                }}
                label="Item Origin Name"
                required
                error={errors.itemOriginName}
                maxLength={15}
              />
            </div>
          </ModalBody>
        ...
        {/* Confirm Modal (for status changes and create) */}
        <ConfirmModal
          isOpen={showConfirm}
