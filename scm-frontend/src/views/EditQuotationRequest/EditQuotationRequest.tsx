...
import { useEntityFormatter } from "../Entity/UseEntityFormater"; // ✅ import formatter
...
const EditQuotationrequest = () => {
  ...
  >([]);
  const [requisitionOptions, setRequisitionOptions] = useState<any[]>([]);
  ...
  // Reset main page when filter changes
  useEffect(() => {
    setMainCurrentPage(1);
  }, [globalFilter]);

  // Paginated data for main table
  ...
  // Add "Select requisition" option at the top of dropdown
  const reqOptions = useMemo(() => {
    const selectOption = {
      label: "Select requisition",
      value: "select-requisition",
      pk: "select-requisition",
      locationId: "",
    };
    ...
    const fetchRequisitions = async () => {
      ...
      try {
        const { data } = await axios.get(
          `http://43.254.31.234:9070/api-gateway-scm/tender-process-service-scm//tenderProcessController/dropDownQuotationReqNo?period=${apiDate}`,
          {
            headers: {
  ...
  const fetchItems = async (locationId: string) => {
    ...
    try {
      const { data } = await axios.get(
        `http://43.254.31.234:9070/api-gateway-scm/tender-process-service-scm/tenderProcessController/loadAPLForLocationRequest/${locationId}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
      ...
      if (data.success) {
        const mapped: TableTypeDense[] = data.data.map((i: any) => ({
          ...
          supplierName: i.supplierName || "N/A",
          status: "Active",
          budget: formatter.formatAmount(i.grandTotal?.toString() || "0.0"),
          isSelected: false,
          totalQty: "0.0",
          quantities: {},
        }));
  ...
  const fetchRequestDetails = async (pk: number, locationId: string) => {
    ...
    try {
      const { data } = await axios.get(
        `http://43.254.31.234:9070/api-gateway-scm/tender-process-service-scm//tenderProcessController/getRequestDetails/${pk}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleSelect = (option: any) => {
    // If "Select requisition" clicked → clear everything
    if (option.pk === "select-requisition") {
  ...
  const handleSelectAllDates = () => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      newDates.push({
        date: `${String(d).padStart(2, "0")}-${String(periodMonth).padStart(
          2,
          "0"
        )}-${periodYear}`,
        day: d,
  ...
  // Controlled quantity input with validation: 5 digits before decimal, decimal capped at min(user setting,5)
  const handleQuantityInputChange = useCallback(
    (itemId: number, date: string, value: string) => {
      ...
      const decimalSetting = getQuantityDecimalPlaces(); // from localStorage
  ...
  // Effect to focus after page change
  useEffect(() => {
  ...
  const handleOpenItemsModal = () => {
    ...
    setOpenModal(true);
    setFocusedRowIndex(-1); // reset keyboard focus
  };
  ...
  const validateSave = (): boolean => {
    ...
    if (!locationId || locationId.trim() === '') {
      toast.error("Please enter Requisition ID.");
      return false;
    }
    ...
    const hasNonZeroQuantity = selectedItems.some((item) =>
      selectedDates.some(
        (dateObj) => parseFloat(item.quantities?.[dateObj.date] || "0.0") > 0
      )
    );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const requestBody = {
        ...
        subList: selectedItems.map((item, index) => {
          const itemData: any = {
            ...
            entOrder: index + 1,
            deliveryMode: "DD",
            qty: parseQuantityInput(calculateTotalQuantity(item))
          };
      ...
      const response = await axios.post(
        "http://43.254.31.234:9070/api-gateway-scm/tender-process-service-scm//tenderProcessController/updateQuotationRequestProcess",
        requestBody,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const defaultColumns = useMemo(
    () => [
      ...
      columnHelper.accessor("itemName", {
        ...
        cell: (info) => (
          <p className="font-medium text-[11px] text-black dark:text-white max-w-[180px]" title={info.getValue()}>
            {info.getValue()}
          </p>
        ),
        sortingFn: "alphanumeric",
      ...
      columnHelper.accessor("totalQty", {
        ...
        cell: (info) => (
          <div className="text-right font-medium text-[11px] dark:text-white ml-10">
            {formatter.formatQuantity(parseFloat(info.getValue() || "0.0"))}
          </div>
        ),
    ...
    [selectedDates, handleQuantityInputChange, handleQuantityKeyDown, formatter, mainCurrentPage, mainPageSize]
  );
  ...
  const modalColumns = [
    ...
    columnHelper.accessor("itemName", {
      ...
      cell: (info) => (
        <p className="font-medium text-[11px] dark:text-white max-w-[180px]" title={info.getValue()}>
          {info.getValue()}
        </p>
      ),
      sortingFn: "alphanumeric",
  ...
  let content;
  if (showTable) {
    // content = <QuotationReqTable onBack={() => setShowTable(false)} />;
  } else {
    content = (
            ...
            <div className="p-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                  {/* Modern Requisition Dropdown - Updated to match Quotationrequest */}
                        ...
                        >
                          <div className="flex flex-col items-center justify-center">
                            ...
                            <p className="text-gray-500 dark:text-gray-400 text-[9px] max-w-md">
                              Click "Select Items" to add items to your quotation request.
                            </p>
                          </div>
                        </td>
                      </tr>
        ...
        >
          <ModalHeader className="border-b border-gray-200 dark:border-gray-700 p-2 bg-white dark:bg-gray-800">
            <div className="flex items-center justify-between w-full">
              ...
              {/* Right: Action buttons (Add, Cancel, Close) */}
          ...
          <ModalBody className="p-3 bg-white dark:bg-gray-800">
            {/* Top bar: Selected count left, search right */}
          ...
          <ModalBody className="p-3 bg-white dark:bg-gray-800">
            <div className="space-y-3">
              ...
              <p className="text-xs text-gray-700 dark:text-gray-300 text-center">
                Are you sure you want to save this quotation request?
              </p>
            </div>
          </ModalBody>
