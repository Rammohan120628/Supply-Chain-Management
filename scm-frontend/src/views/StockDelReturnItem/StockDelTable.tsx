...
const simpleFilterFn: FilterFn<TableTypeDense> = (row, columnId, filterValue) => {
...
const ReturnItemTable = ({ onBack }) => {
  ...
  const fetchData = async (periodParam: string) => {
    ...
    setIsLoading(true);
    try {
      const apiUrl = `http://43.254.31.234:9070/api-gateway-scm/stock-movement-analysis-scm/stockDeliveryController/listReturnItemsHeader/${periodParam}`;
  ...
  const fetchSubListData = async (suppReturnId: string) => {
    ...
    try {
      setIsLoading(true)
      const apiUrl = `http://43.254.31.234:9070/api-gateway-scm/stock-movement-analysis-scm/stockDeliveryController/listReturnItemsToSupplierSub/${suppReturnId}`;
  ...
  const exportToExcel = async () => {
    ...
    try {
      const excelApiUrl = `http://43.254.31.234:9070/api-gateway-scm/stock-movement-analysis-scm/stockDeliveryController/downloadReturnItemsToSupplier/${period}`;
      ...
      let filename = `ReturnItemsToSupplier.xlsx`;
  ...
  const defaultColumns1 = [
    columnHelper.accessor('siNo', {
      ...
      cell: (info) => <span className="text-[11px] text-black   w-full block">{info.getValue()}</span>,
      size: 65,
    }),
    columnHelper.accessor('returnId', {
      ...
      cell: (info) => (
        <span className="text-[11px] font-medium text-black break-words block max-w-[120px]">
          {info.getValue()}
        </span>
      ),
      size: 120,
    ...
    columnHelper.accessor('supplierName', {
      ...
      cell: (info) => (
        <div className="min-w-[100px]">
          <div className="text-[11px] font-medium leading-tight text-black break-all">{info.row.original.supplierId}</div>
    ...
    columnHelper.accessor('locationId', {
      ...
      cell: (info) => (
        <div className="min-w-[100px]">
          <div className="text-[11px] text-black font-medium leading-tight break-all">{info.row.original.locationId}</div>
          <div className="text-[10px] text- leading-tight break-all">
            {info.row.original.locationName || 'N/A'}
          </div>
        </div>
      ),
      size: 120,
    ...
    columnHelper.accessor('userId', {
      ...
      cell: (info) => (
        <div className="min-w-[100px]">
          <div className="text-[11px] font-medium leading-tight text-black break-all">
            {info.row.original.userId.replace(/\.(\w+)/, '.\u200B$1')}
          </div>
          <div className="text-[9px]  leading-tight break-all">
    ...
    columnHelper.accessor(row => row.returnItem?.itemId, {
      ...
      cell: (info) => (
        <div className="min-w-[100px]">
          ...
          <div className="text-[10px] text-black">{info.row.original.returnItem?.itemName || 'N/A'}</div>
        </div>
      ),
      enableSorting: true,
      sortingFn: 'basic',
  ...
  const DashboardCards = () => {
    ...
    const SmallCard = [
      ...
      },
      {
        ...
        desc: "Returns older than 30 days",
      },
    ];
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-black mt-0.5 font-bold truncate">
                        <span className='text-blue-600'>Supplier: </span>{item.supplierId || 'N/A'}
                      </p>
                    </div>
                    <button
                  ...
                  <div className="grid grid-cols-3 gap-1 sm:gap-2 mb-2">
                    ...
                    <div className="bg-purple-50 p-1.5 sm:p-2 rounded">
                      <div className="flex items-center gap-1 mb-0.5">
                        ...
                        <span className="text-xs text-purple-600">Net Inv</span>
                      </div>
                      <p className="text-xs sm:text-sm font-bold text-gray-800 truncate">
  ...
  return (
    <div className="w-full max-w-full mx-auto px-2 sm:px-2 pb-2 sm:pb-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 mb-4">
            ...
            <div className="w-full sm:flex-1 relative" ref={periodRef}>
              <div className="relative">
                ...
                <CalendarDays className="absolute right-45 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4 sm:w-5 sm:h-5 pointer-events-none" />
              </div>
            ...
            <div className="relative w-full sm:flex-1">
              <div className="relative">
                <input
                  ...
                  className="form-control-input w-80 lg:ml-60 px-3 sm:px-4 py-2 text-xs sm:text-sm pr-10 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500"
                  disabled={isLoading}
                  value={search}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
            ...
            {period && (
             ...
             <input
                ...
                className="lg:ml-130 form-control-input w-full sm:w-60 px-3 py-1.5 text-sm border rounded-md"
                autoFocus
              />
              {modalSearch && (
                <button
