...
const defaultColumns = (formatter: ReturnType<typeof useEntityFormatter>) => [
  columnHelper.accessor('name', {
    ...
    header: ({ column }) => (
      ...
      >
        <span className="font-medium text-white text-[10px] uppercase">Quotation No</span>
        {column.getCanSort() && (
  ...
  columnHelper.accessor('supplierId', {
    ...
    cell: (info) => (
      <div className="text-[11px] font-bold text-black dark:text-white dark:bg-gray-800">
        {info.getValue()}
      </div>
    ),
  }),
  ...
  columnHelper.accessor('entityId', {
    ...
    header: ({ column }) => (
      <div
        ...
        onClick={column.getToggleSortingHandler()}
      >
        <span className="font-medium text-white text-[10px] uppercase">Entity ID</span>
        {column.getCanSort() && (
          <span className="text-[10px]">
            {{
              asc: ' 🔼',
...
// Modal columns with combined Item ID and Item Name
  ...
  columnHelper.accessor('net', {
    ...
    header: ({ column }) => (
      <div
        ...
        onClick={column.getToggleSortingHandler()}
      >
        <span className="font-medium text-white text-[10px] uppercase">Net</span>
        {column.getCanSort() && (
          <span className="text-[10px]">
            {{
              asc: ' 🔼',
...
const defaultColumns2Reduced = (formatter: ReturnType<typeof useEntityFormatter>) => {
  ...
  return [full[0], full[1], full[2], full[3]]; // Item ID, Item Name, Package ID, Quantity
};

const QuotationReplyTable = ({ onBack }: { onBack?: () => void }) => {
  ...
  const mapApiDataToTable = (apiData: any[]): TableTypeDense[] => {
    return apiData.map((item) => ({
      ...
      quotationProcessHeadPk: item.quotationProcessHeadPk,
      entityId: item.entityId || 'Unknown',
      statuscolor: item.consolidationId ? 'success' : 'warning',
  ...
  const fetchData = async (period?: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://43.254.31.234:9070/api-gateway-scm/tender-process-service-scm/tenderProcessController/listOfQuotationReply/${finalPeriod}`,
        {
          method: 'GET',
          headers: {
      ...
      if (result.success) {
        setQuotationReplyListData(mapApiDataToTable(result.data.quotationPreparedList || []));
        setFinalizedListData(mapApiDataToTable(result.data.finalizedList || []));
      } else {
  ...
  const fetchItemDetails = async (pk: number) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://43.254.31.234:9070/api-gateway-scm/tender-process-service-scm/tenderProcessController/subListOfQuotationReply/${pk}`,
        {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleViewClick = (rowData: TableTypeDense, sourceTab: number) => {
    if (rowData.quotationProcessHeadPk) {
      setViewSourceTab(sourceTab);
      fetchItemDetails(rowData.quotationProcessHeadPk);
      setOpenModal(true);
    }
  };
  ...
  // Add view columns (with sorting disabled)
          ...
          cell: ({ row }) => (
            <Tooltip content="View item details" className="dark:bg-gray-800 dark:text-white">
              <button
                ...
                onClick={() => handleViewClick(row.original, activeTab)}
                title="View Details"
              >
  ...
  const getModalTitle = () => {
    switch (viewSourceTab) {
      case 0:
        return 'Item Details – Quotation Prepared';
      case 1:
        return 'Item Details – Finalized (Supplier Selected)';
      default:
        return 'Item Details';
    }
  };
    ...
    <div className="w-[1050px]">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md p-2 sm:p-4">
        <div className="w-full">
            ...
            <div className="w-full sm:max-w-xs">
              <div className="relative text-gray-400 focus-within:text-gray-600">
                ...
                <input
                  ...
                  className="form-control-input w-74 pl-8 px-3 py-2 sm:py-2.5 border rounded-md text-sm"
                  value={globalSearch}
          ...
          {/* Tabs */}
          <Tabs
            aria-label="Quotation reply tabs"
            variant="underline"
...
export default QuotationReplyTable;