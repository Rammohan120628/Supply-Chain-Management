...
import { FaSave, FaSort, FaSortUp, FaSortDown, FaMapMarkerAlt, FaCalendarAlt, FaChevronLeft, FaChevronRight } from "react-icons/fa";
...
const ChangetheDeliveryLocationSupplier = () => {
  ...
  const uniqueSuppliers = useMemo(() => {
    return new Set(data.map(item => item.supplierId)).size;
  }, [data]);

  const uniqueLocations = useMemo(() => {
    return new Set(data.map(item => item.locationId)).size;
  }, [data]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const month = (purchasePeriod.getMonth() + 1).toString().padStart(2, '0');
      const year = purchasePeriod.getFullYear();
      const apiDate = `01-${month}-${year}`;

      const response = await fetch(
        `http://43.254.31.234:9070/api-gateway-scm/request-and-po-service-scm/requestAndPOController/viewPeriodBasedLocationRequestSupBasedProcess2/${apiDate}`,
        {
          method: 'GET',
          headers: {
  ...
  const performSave = async () => {
    ...
    try {
      ...
      const savePayload = data.map(item => ({
        reqTransactionNo: item.reqTransactionNo,
        supplierId: item.supplierId,
      ...
      const response = await fetch(
        'http://43.254.31.234:9070/api-gateway-scm/request-and-po-service-scm/requestAndPOController/updateDeliveryLocationSupBasedProcess2',
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  // Handle checkbox change using unique reqTransactionNo
  const handleCheckboxChange = (reqTransactionNo: string, checked: boolean) => {
    setData(prevData =>
      prevData.map(item =>
        item.reqTransactionNo === reqTransactionNo
    ...
    columnHelper.accessor("supplierId", {
      header: ({ column }) => (
        <div
          ...
          onClick={() => column.toggleSorting()}
        >
          <span className="font-medium text-white text-[10px] uppercase tracking-wider">Supplier Id</span>
          {column.getIsSorted() === "asc" ? (
            <FaSortUp className="w-2 h-2 ml-0.5" />
          ) : column.getIsSorted() === "desc" ? (
            <FaSortDown className="w-2 h-2 ml-0.5" />
      ...
      cell: (info) => <p className="text-[10px] text-black font-bold dark:text-gray-400">{info.getValue() || '-'}</p>,
      sortingFn: 'alphanumeric',
    }),
    columnHelper.accessor("supplierName", {
      header: ({ column }) => (
        <div
          ...
          onClick={() => column.toggleSorting()}
        >
          <span className="font-medium text-white text-[10px] uppercase tracking-wider">Supplier Name</span>
          {column.getIsSorted() === "asc" ? (
            <FaSortUp className="w-2 h-2 ml-0.5" />
          ) : column.getIsSorted() === "desc" ? (
            <FaSortDown className="w-2 h-2 ml-0.5" />
      ...
      cell: (info) => <p className="text-[10px] text-black font-bold dark:text-gray-400  max-w-[200px]">{info.getValue() || '-'}</p>,
      sortingFn: 'alphanumeric',
    }),
      ...
      cell: (info) => (
        <div className="flex items-center justify-center ml-9">
          <input
            ...
            onChange={(e) => handleCheckboxChange(info.row.original.reqTransactionNo, e.target.checked)}
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4">
          <div className="flex items-center gap-2">
            <h1 className="text-lg md:text-xl font-bold text-gray-900 dark:text-indigo-400 flex items-center gap-2">
              Change Delivery Location-Supplier
            </h1>
            <Tooltip
              content={
                <div className="text-xs max-w-xs">
                  ...
                  <ol className="list-decimal list-inside space-y-1">
                    <li>Review supplier locations for the period</li>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-1 bg-white dark:bg-gray-800 px-2 py-1 rounded-md shadow-xs border border-gray-200 dark:border-gray-700">
                  ...
                  <Tooltip
                    content="Purchase period for supplier-based delivery locations"
                    placement="top"
                    className="dark:bg-gray-800 dark:text-white z-50"
                  >
          ...
          <div className="p-3">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-2 mb-2">
              <h3 className="text-xs font-semibold text-gray-900 dark:text-white flex items-center gap-1">
                <FaMapMarkerAlt className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />
                Delivery Locations for Suppliers
                {data.length > 0 && (
                      ...
                      <td colSpan={table.getAllColumns().length} className="px-1.5 py-6 text-center">
                        <div className="flex flex-col items-center justify-center">
                          <div className="p-1.5 bg-blue-100 dark:bg-blue-900/30 rounded-full mb-1">
                            <FaMapMarkerAlt className="w-5 h-5 text-blue-400 dark:text-blue-300" />
                          </div>
                          ...
                          <p className="text-gray-500 dark:text-gray-400 text-[9px] max-w-md">
                            No delivery locations found for this period.
                          </p>
                        </div>
                      </td>
                    </tr>
...
export default ChangetheDeliveryLocationSupplier;