...
const SuppliersInvoiceDetailsByInvoiceSummary: React.FC = () => {
  // --- Dark mode ---
  ...
  const [selectedSupplierName, setSelectedSupplierName] = useState<string>('All Supplier');
  const [supplierDropdownOpen, setSupplierDropdownOpen] = useState<boolean>(false);
  const [supplierSearchTerm, setSupplierSearchTerm] = useState<string>('');

  const [invoices, setInvoices] = useState<Invoice[]>([]);
  const [selectedInvoiceId, setSelectedInvoiceId] = useState<string>('');
  const [selectedInvoiceNo, setSelectedInvoiceNo] = useState<string>('All Invoice No');
  const [invoiceDropdownOpen, setInvoiceDropdownOpen] = useState<boolean>(false);
  ...
  const [deliveryType, setDeliveryType] = useState<'CWH' | 'DD' | 'BOTH'>('BOTH');
  ...
  const fetchDropdownData = async () => {
    ...
    try {
      const res = await fetch(
        `http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/pythonReportController/dropDownPeriodBasedSupplierIdBySupplierInvoiceDetailsByInvoiceReport?period=${period}`,
        {
          method: 'GET',
          headers: {
      ...
      if (result.success) {
        ...
        const { invoiceNos, suppliers: apiSuppliers } = result.data || { invoiceNos: [], suppliers: [] };

        // Suppliers
        const allSupplier: Supplier = { supplierId: '', supplierName: 'All Supplier' };
        if (apiSuppliers && apiSuppliers.length > 0) {
          const supplierList = [allSupplier, ...apiSuppliers
            .filter((item: any) => item.supplierId && item.supplierName)
            .map((item: any) => ({
          ...
          setSuppliers([allSupplier]);
        }

        // Invoices
        const allInvoice: Invoice = { invoiceId: '', invoiceNo: 'All Invoice No' };
        if (invoiceNos && invoiceNos.length > 0) {
          const invoiceList = [allInvoice, ...invoiceNos
          ...
          showToast('No invoices available for the selected period', 'error');
          setInvoices([allInvoice]);
        }
      } else {
        showToast(result.message || 'Failed to load dropdown data', 'error');
        const allSupplier: Supplier = { supplierId: '', supplierName: 'All Supplier' };
        const allInvoice: Invoice = { invoiceId: '', invoiceNo: 'All Invoice No' };
        setSuppliers([allSupplier]);
        setInvoices([allInvoice]);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (supplierRef.current && !supplierRef.current.contains(event.target as Node)) setSupplierDropdownOpen(false);
      if (invoiceRef.current && !invoiceRef.current.contains(event.target as Node)) setInvoiceDropdownOpen(false);
  ...
  const filteredSuppliers = suppliers.filter((sup) =>
    sup.supplierName.toLowerCase().includes(supplierSearchTerm.toLowerCase())
  );
  const filteredInvoices = invoices.filter((inv) =>
    inv.invoiceNo.toLowerCase().includes(invoiceSearchTerm.toLowerCase())
  );

  // --- Period picker UI (unchanged) ---
  ...
  const generateFileName = (): string => {
    ...
    let fileName = `SupplierInvoiceDetailsByInvoiceSummaryReport_${periodStr}`;
    if (selectedSupplierName && selectedSupplierName !== 'All Supplier') {
      fileName += `_${selectedSupplierName.replace(/\s+/g, '_')}`;
    }
    if (selectedInvoiceNo && selectedInvoiceNo !== 'All Invoice No') {
      fileName += `_${selectedInvoiceNo.replace(/\s+/g, '_')}`;
    }
    if (fromDate) fileName += `_from_${fromDate.format('DD-MM-YYYY')}`;
    if (toDate) fileName += `_to_${toDate.format('DD-MM-YYYY')}`;
    fileName += `_${deliveryType}`;
    return `${fileName}.pdf`;
  };

  const buildRequestBody = () => {
    const reportType = deliveryType === 'CWH' ? 1 : deliveryType === 'DD' ? 2 : 3;
    const body: any = {
      month: selectedMonth + 1,
      year: selectedYear,
      deliveryType: reportType,
    };
    if (selectedSupplierId?.trim()) body.supplierId = selectedSupplierId;
    if (selectedInvoiceId?.trim()) body.invoiceNo = selectedInvoiceId;
    if (fromDate?.isValid()) body.fromDate = fromDate.format('YYYY-MM-DD');
    if (toDate?.isValid()) body.toDate = toDate.format('YYYY-MM-DD');
    return body;
  };
  ...
  const handleDownload = async () => {
    ...
    try {
      const res = await fetch(
        'http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/scmReportsController/listOfSupplierInvoiceDetailsByInvoiceSummaryPdf',
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleClear = () => {
    ...
    setSelectedSupplierName('All Supplier');
    setSupplierSearchTerm('');
    setSelectedInvoiceId('');
    setSelectedInvoiceNo('All Invoice No');
    setInvoiceSearchTerm('');
    setDeliveryType('BOTH');
    const now = dayjs('2025-12-10');
  ...
  // ----------------------------------------------------------------------
  // Render (only DatePickers are changed – everything else stays identical)
  // ----------------------------------------------------------------------
  return (
    <div className="max-h-screen w-full bg-gray-50 dark:bg-gray-900 flex flex-col transition-all duration-300 p-2 sm:p-4">
        ...
        <div className="fixed inset-0 bg-transparent backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md mx-auto">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              Do you want to download the Supplier Invoice Details By Invoice Summary Report for the selected criteria?
            </p>
      ...
      {/* Header (unchanged) */}
      <CommonHeader
        title="Supplier Invoice Details By Invoice Summary"
      ...
      <div className="bg-white dark:bg-gray-800 p-2 sm:p-4 md:p-6 lg:p-8 rounded-2xl shadow-md w-full max-w-6xl mx-auto">
        <LocalizationProvider dateAdapter={AdapterDayjs}>
          <div className="space-y-4 sm:space-y-6">
            {/* Row 1: Period, Supplier, Delivery Type (unchanged) */}
            <div className="flex flex-col xl:flex-row gap-3 sm:gap-4 items-end">
              {/* Period Picker (unchanged) */}
              ...
              {/* Delivery Type Radios (unchanged) */}
              <div className="flex-1 min-w-0">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Delivery Type
                </label>
                <div className="flex gap-4 bg-gray-50 dark:bg-gray-700 p-2 rounded-md">
                  <label className="flex items-center cursor-pointer text-xs sm:text-sm">
                    <input
                      type="radio"
                      value="CWH"
                      checked={deliveryType === 'CWH'}
                      onChange={() => setDeliveryType('CWH')}
                  ...
                  <label className="flex items-center cursor-pointer text-xs sm:text-sm">
                    <input
                      type="radio"
                      value="DD"
                      checked={deliveryType === 'DD'}
                      onChange={() => setDeliveryType('DD')}
                      className="mr-1 text-blue-600 focus:ring-blue-500"
                    />
                    DD
                  </label>
            ...
            {/* Row 2: Invoice No, From Date, To Date – only DatePickers changed */}
            <div className="flex flex-col lg:flex-row gap-3 sm:gap-4">
              {/* Invoice No Dropdown (unchanged) */}
              <div className="flex-1 relative" ref={invoiceRef}>
                <input
                  id="invoice"
                  type="text"
              ...
              {/* --- FIXED: To Date (exact copy from reference) --- */}
              <div className="flex-1">
                <DatePicker
...
export default SuppliersInvoiceDetailsByInvoiceSummary;