...
interface Family {
  pk: number;
  familyId: string; // itemCode
  familyName: string; // name
}

const TheoriticalStockPdf: React.FC = () => {
  ...
  // Function to download the file (UPDATED: New endpoints for Theoretical Stock; Include family name in filename)
  const downloadFile = async () => {
    ...
    try {
      ...
      if (selectedFamily) {
        ...
        apiUrl = `http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/scmReportsController/listOfTheoreticalDataFamilyIdBased/${familyId}/${userId}`;
      } else {
        apiUrl = `http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/scmReportsController/listOfTheoreticalDataByTheoriticalStockReportNew/${userId}`;
      }
      ...
      let filename = selectedFamily
        ? `TheoreticalStock-${getSelectedFamilyId()}-${getSelectedFamilyName().replace(/[^a-zA-Z0-9]/g, '_')}${fileExtension}`
        : `TheoreticalStock-AllFamilies${fileExtension}`;
  ...
  const getModalMessage = () => {
    if (selectedFamily && familyName) {
      return `Are you sure you want to download the Theoritical Stock Report PDF for ${familyName}?`;
    } else {
      return `Are you sure you want to download the Theoritical Stock Report PDF for all families?`;
    }
  };

  return (
    <div className="dark:bg-gray-900 max-h-screen w-full bg-gray-50 flex 
    flex-col p-4 transition-all duration-300">
      ...
      {/* Confirmation Modal */}
      {showConfirmModal && (
        <div className="fixed inset-0 bg-transperant bg-opacity-50 
        dark:bg-opacity-70 backdrop-blur-sm flex items-center justify-center z-40 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-4 sm:p-6 w-full max-w-md border border-gray-200 dark:border-gray-700">
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
              <button
                ...
                className="px-4 py-2 text-gray-600 dark:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-sm sm:text-base order-2 sm:order-1"
              >
                Cancel
              </button>
              <button
      ...
      {/* Header */}
      <CommonHeader
      title="Theoritical Stock Report PDF"
              icon={<FileText className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8" />}/>

      {/* Main Content */}
      <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 md:p-8 rounded-2xl shadow-md w-full max-w-6xl mx-auto flex-1 border border-gray-200 dark:border-gray-700">
        {/* Form Controls */}
                ...
                <div className="p-3 border-b border-gray-200 dark:border-gray-600">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 pl-10 py-2 text-sm focus:ring-2 focus:ring-blue-400 outline-none transition-all duration-200 bg-white dark:bg-gray-600 text-gray-700 dark:text-gray-300 placeholder-gray-400 dark:placeholder-gray-500"
                    />
                  </div>
                </div>
                  ...
                  {filteredFamilies.length > 0 ? (
                    filteredFamilies.map((family) => (
                      <li
                        ...
                        className="px-4 py-3 cursor-pointer hover:bg-blue-50 dark:hover:bg-gray-600 border-b border-gray-100 dark:border-gray-600 last:border-b-0 transition-colors duration-200"
                      >
...
export default TheoriticalStockPdf;