...
const CwhSavingsReport: React.FC = () => {
  ...
  // Fetch locations from new API based on period
  const fetchLocations = async () => {
    try {
      ...
      const apiUrl = `http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/pythonReportController/dropDownPeriodBasedLocationIdBySavings?period=${formattedDate}`;
      ...
      if (result.success && result.data) {
        ...
        setLocations(result.data.map((item: any) => ({
          pk: item.pk,
  ...
  // Function to format date for GET API (DD-MM-YYYY with 01 as default day)
  const formatDateForApi = (month: number, year: number): string => {
    const formattedMonth = String(month + 1).padStart(2, '0');
    return `01-${formattedMonth}-${year}`;
  };

  // Function to format period for POST API (YYYY-MM-01)
  const formatPeriodForPost = (month: number, year: number): string => {
    const formattedMonth = String(month + 1).padStart(2, '0');
  ...
  const getSelectedLocationId = (): string => {
    ...
    const selectedLocationObj = locations.find(
      (loc) => formatLocationDisplay(loc) === selectedLocation,
    );

    return selectedLocationObj
      ? selectedLocationObj.locationId
      : extractLocationId(selectedLocation);
  };
  ...
  // Function to download the file using new POST API
  const downloadFile = async () => {
    setLoading(true);
    setError('');

    try {
      const postPeriod = formatPeriodForPost(selectedMonth, selectedYear);
      const token = localStorage.getItem('authToken');
      ...
      const response = await fetch('http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/pythonReportController/cwhSavingsReport', {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      const fileExtension = reportType === 1 ? '.pdf' : '.xlsx';
      ...
      let filename = `CWH-Savings-Report-${locationId}-${months[selectedMonth]} ${selectedYear}${fileExtension}`;
  ...
  const getModalMessage = () => {
    const fileType = reportType === 1 ? 'PDF' : 'Excel';
    const period = `${months[selectedMonth]} ${selectedYear}`;
    const locationName = selectedLocation.split(' ').slice(1).join(' ');
    return `Are you sure you want to download the CWH Savings Report (${fileType}) for ${locationName} for ${period}?`;
  };
...
export default CwhSavingsReport;