...
const CwhDeliveryDetailsByLocationReport: React.FC = () => {
  ...
  const fetchLocations = async () => {
    try {
      ...
      const response = await fetch(
        `http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/pythonReportController/dropDownPeriodBasedLocationIdByCwhDeliveryDetailsByLocationReport/${period}`,
        {
          method: 'GET',
          headers: {
  ...
  const downloadFile = async () => {
    ...
    try {
      ...
      const apiUrl = `http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/pythonReportController/cwhDeliveryByLocationReport?period=${formattedDate}&locationId=${locationId}`;
      ...
      let filename = `CwhDeliveryDetailsByLocationReport_${locationId}_${formattedDate}${fileExtension}`;
  ...
  const getModalMessage = () => {
    ...
    if (selectedLocation) {
      ...
      return `Are you sure you want to download the CWH Delivery Details By Location Report for ${locationName} for ${period}?`;
    } else {
      return `Are you sure you want to download the CWH Delivery Details By Location Report for the selected period?`;
    }
  };
  ...
  return (
    <div className="max-h-screen w-full bg-gray-50 dark:bg-gray-900 flex flex-col p-2 sm:p-4 transition-all duration-300">
      ...
      {/* Header */}
       ...
       <CommonHeader
      title="CWH Delivery Details By Location Report"
              ...
              <div className="absolute top-full mt-2 w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-xl z-20 overflow-hidden">
                ...
                <ul className="max-h-48 sm:max-h-60 overflow-y-auto">
                  {filteredLocations.length > 0 ? (
                    ...
                    <li className="px-4 py-4 text-gray-500 dark:text-gray-400 text-sm text-center">
                      No locations available for this period
                    </li>
                  ) : (
                    <li className="px-4 py-4 text-gray-500 dark:text-gray-400 text-sm text-center">
                      No locations found
...
export default CwhDeliveryDetailsByLocationReport;