...
const CwhDeliveryDetailsByInvoice: React.FC = () => {
  ...
  // Fetch locations based on selected period
  const fetchLocations = async (period: string) => {
    try {
      ...
      const apiUrl = `http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/pythonReportController/dropDownPeriodBasedLocationIdByCwhDeliveryDetailsByInvoiceReport?period=${period}`;
      ...
      if(response.ok && response.status == 200 && result?.data?.length == 0){
        showToast('No Locations data fetched for selected period','error')
      }
      if(result.data.length){
        showToast('Location Dropdown loaded','success')
      }
  ...
  // Fetch locations when period changes
  useEffect(() => {
    const formattedPeriod = formatDateForApi(selectedMonth, selectedYear);
    fetchLocations(formattedPeriod);
  ...
  // Function to extract location ID from display string
  const getLocationIdFromDisplay = (display: string): string => {
  ...
  // Function to format date for POST API (YYYY-MM-DD format)
  const formatDateForPostApi = (month: number, year: number): string => {
    const formattedMonth = String(month + 1).padStart(2, '0');
  ...
  const downloadFile = async () => {
    ...
    try {
      const formattedDate = formatDateForPostApi(selectedMonth, selectedYear);
      const token = localStorage.getItem('authToken');
      ...
      // Add locationId only if a location is selected
      if (selectedLocation) {
        requestBody.locationId = getLocationIdFromDisplay(selectedLocation);
      }

      console.log('Request body:', requestBody);

      const response = await fetch(
        'http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/pythonReportController/cwhDeliveryDetailsByInvoiceReport',
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (contentType.includes('application/json')) {
        ...
        if (!result.success && result.message) {
          // Specific handling for no data
          const noDataMessage =
            result.message.includes('no record') || result.message.includes('No records') || result.message.includes('no data')
              ? 'No records found for the selected period and location.'
              : result.message;
      ...
      let filename = selectedLocation
        ? `CWH-Delivery-Details-${getLocationIdFromDisplay(selectedLocation)}-${formattedDate}${fileExtension}`
        : `CWH-Delivery-Details-All-Locations-${formattedDate}${fileExtension}`;
  ...
  const getModalMessage = () => {
    ...
    if (selectedLocation) {
      const locationName = selectedLocation.split(' - ')[1];
      return `Are you sure you want to download the CWH Delivery Details report for ${locationName} for ${period}?`;
    } else {
      return `Are you sure you want to download the CWH Delivery Details report for all locations for ${period}?`;
    }
  };
  ...
  return (
    <div className="max-h-screen w-full bg-gray-50 dark:bg-gray-900 flex flex-col p-2 sm:p-4 transition-all duration-300">
      ...
      {/* Confirmation Modal */}
      {showConfirmModal && (
        <div className="fixed inset-0 bg-transparent backdrop-blur-sm bg-black/50 flex items-center justify-center z-40 p-4">
      ...
      {/* Header */}
       ...
       <CommonHeader
            title="CWH Delivery Details By Invoice Report"
              icon={<ScrollText className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8" />}
            />
          ...
          <div className="flex flex-col w-full lg:w-1/3 relative" ref={locationRef}>
            <div className="relative mt-2">
              <input
                ...
                type="text"
                placeholder=" "
                value={loadingLocations ? 'Loading locations...' : selectedLocation || ''}
                readOnly
            ...
            {dropdownOpen && !loadingLocations && (
              <div className="absolute top-full mt-2 w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-xl z-20 overflow-hidden">
                ...
                <ul className="max-h-48 sm:max-h-60 overflow-y-auto">
                  <li key='' className="px-4 py-3 cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900/20 border-b border-gray-100
                   dark:border-gray-700 last:border-b-0 transition-colors duration-200"
                      >Please Select Location id</li>
...
export default CwhDeliveryDetailsByInvoice;