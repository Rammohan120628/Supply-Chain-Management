...
const Consolidation: React.FC = () => {
  ...
  const [loadingConsolidations, setLoadingConsolidations] = useState<boolean>(true);
  const [isLoading, setIsLoading] = useState<boolean>(true); // New global loader state
  ...
  // Filter consolidation items based on search term
  const filteredConsolidationItems = useMemo(() => {
    if (!searchTerm.trim()) {
      return consolidationItems;
    }
    const searchLower = searchTerm.toLowerCase();
    return consolidationItems.filter(item =>
      item.consolidationId.toLowerCase().includes(searchLower)
    );
  }, [consolidationItems, searchTerm]);

  // Fetch consolidation IDs from API based on period
  useEffect(() => {
    const fetchConsolidations = async () => {
      ...
      try {
        ...
        const apiUrl = `http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/pythonReportController/dropDownPeriodBasedConsolidationId?period=${formattedDate}`;
        ...
        if (result.success && result.data) {
          setConsolidationItems(result.data);
          if (result.data.length === 0) {
            showToast('No consolidation IDs found for the selected period.', 'error');
          } else {
            showToast('Consolidation IDs Loaded.', 'success');
          }
        } else {
          throw new Error(result.message || 'Failed to fetch consolidation IDs');
        }
      } catch (err) {
        console.error('Error fetching consolidation IDs:', err);
        const errorMessage = err instanceof Error ? err.message : 'Failed to load consolidation IDs';
        showToast(errorMessage, 'error');
        setConsolidationItems([]);
        handleSessionExpired();
      } finally {
        setLoadingConsolidations(false);
        setIsLoading(false);
      }
    };
  ...
  const downloadFile = async () => {
    if (!selectedConsolidationId) {
      showToast('Please select a Consolidation ID', 'error');
      return;
    }
    ...
    try {
      ...
      const apiUrl = `http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/pythonReportController/consolidationReport?${params.toString()}`;
        ...
        if (!result.success && result.message) {
          ...
          const noDataMessage =
            result.message.includes('no record') || result.message.includes('No records')
              ? 'No records found for the selected consolidation ID and period.'
              : result.message;
      ...
      let filename = `Consolidation-Report-${selectedConsolidationId}${fileExtension}`;
  ...
  const getModalMessage = () => {
    ...
    return `Are you sure you want to download the Consolidation report for ${selectedConsolidationId} for ${period}?`;
  }
    ...
    <div className="max-h-screen w-full bg-gray-50 dark:bg-gray-900 flex flex-col p-2 sm:p-4 transition-all duration-300">
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-40 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-4 sm:p-6 w-full max-w-md border border-gray-200 dark:border-gray-700 mx-auto my-auto">
      ...
      <CommonHeader
        ...
        icon={<MapPinPlus className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8" />}
      />
      ...
      <div className="bg-white dark:bg-gray-800 p-2 sm:p-4 md:p-6 lg:p-8 rounded-2xl shadow-md w-full max-w-6xl mx-auto flex-1">
        ...
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-center gap-3 sm:gap-4 md:gap-6 mb-6 sm:mb-8">
          ...
          {/* Consolidation ID Dropdown with Search */}
          <div className="flex flex-col w-full lg:w-1/3 relative" ref={locationRef}>
            <div className="relative mt-2">
              <input
                ...
                type="text"
                placeholder=" "
                value={loadingConsolidations ? 'Loading...' : selectedConsolidationId || ''}
                readOnly
                onClick={() => !loadingConsolidations && setDropdownOpen(!dropdownOpen)}
                disabled={loadingConsolidations}
              ...
              <label
                ...
                className="absolute left-3 top-2 text-gray-500 dark:text-gray-400 transition-all duration-200 pointer-events-none
                          ...
                          peer-[:not(:placeholder-shown)]:text-xs peer-[:not(:placeholder-shown)]:bg-white peer-[:not(:placeholder-shown)]:px-1 dark:peer-[:not(:placeholder-shown)]:bg-gray-800"
              >
                Consolidation ID <sup className='text-red-600'>*</sup>
              </label>
            ...
            {dropdownOpen && !loadingConsolidations && (
              <div className="absolute top-full mt-2 w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-xl z-20 overflow-hidden">
                <div className="p-3 border-b border-gray-200 dark:border-gray-700">
                  <input
                    type="text"
                    placeholder="Search consolidation IDs..."
                    autoFocus
                    value={searchTerm}
                ...
                <ul className="max-h-48 sm:max-h-60 overflow-y-auto">
                  {filteredConsolidationItems.length > 0 ? (
                    filteredConsolidationItems.map((item) => (
                      <li
                        key={item.pk}
                        onClick={() => {
                          setSelectedConsolidationId(item.consolidationId);
                          setDropdownOpen(false);
                          setSearchTerm('');
                        }}
        ...
        <div className="flex justify-center gap-3 sm:gap-4 md:gap-6 pt-4">
          ...
          <button
            ...
            disabled={loading || loadingConsolidations || !selectedConsolidationId}
            className={`bg-gradient-to-r from-blue-600 to-blue-700 dark:from-blue-700 dark:to-blue-800 hover:from-blue-700 hover:to-blue-800 dark:hover:from-blue-800 dark:hover:to-blue-900 
              ...
              ${loading || loadingConsolidations || !selectedConsolidationId ? 'opacity-50 cursor-not-allowed' : ''}`}
...
<Tooltip content='Clear'>
          <button
            ...
            disabled={loading || loadingConsolidations}
      ...
      {/* Global Full-Screen Loader for all API calls and screen render */}
      {isLoading && (
