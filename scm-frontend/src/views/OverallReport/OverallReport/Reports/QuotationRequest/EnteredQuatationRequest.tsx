...
const EnteredQuatationRequest: React.FC = () => {
  const [selectedReqNo, setSelectedReqNo] = useState<string>('');
  ...
  const [loading, setLoading] = useState<boolean>(false);         // for download
  const [showConfirmModal, setShowConfirmModal] = useState<boolean>(false);
  const [showSessionExpiredModal, setShowSessionExpiredModal] = useState<boolean>(false);
  const [dropdownData, setDropdownData] = useState<any[]>([]);
  const [loadingDropdown, setLoadingDropdown] = useState<boolean>(true); // for dropdown fetch

  const locationRef = useRef<HTMLDivElement>(null);
  const periodRef = useRef<HTMLDivElement>(null);

  // Full‑screen loader – active whenever any API call is in progress
  const isLoading = loading || loadingDropdown;
  ...
  const fetchDropdownData = async () => {
    try {
      ...
      console.log('Fetching dropdown data for period:', formattedDate);

      const apiUrl = `http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/pythonReportController/dropDownRequestNoOfLocationRequestHead?period=${formattedDate}`;
      ...
      if (result.success && result.data) {
        ...
        throw new Error(result.message || 'Failed to fetch request numbers');
      }
    } catch (err) {
      console.error('Error fetching dropdown data:', err);
      const errorMessage = 'Failed to load request numbers';
  ...
  const filteredDropdown = dropdownData.filter((item) =>
    item.reqNo.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const downloadFile = async () => {
    ...
    try {
      ...
      if (selectedReqNo && locationId) {
        params.append('reqNo', selectedReqNo);
        params.append('locId', locationId);
      }

      const apiUrl = `http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/pythonReportController/enteredQuatationReqReport?${params.toString()}`;
      console.log('Downloading report with URL:', apiUrl);
      const response = await fetch(apiUrl, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      // Check content-type to handle JSON errors like "no records"
        ...
        if (!result.success && result.message) {
          ...
          const noDataMessage =
            result.message.includes('no record') || result.message.includes('No records')
              ? selectedReqNo
                ? 'No records found for the selected quotation and location.'
      ...
      let filename;
      if (selectedReqNo && locationId) {
        filename = `EnteredQuotationReqReport-${selectedReqNo}-${locationId}-${String(
          selectedMonth + 1
        ).padStart(2, '0')}-${selectedYear}${fileExtension}`;
      } else {
        filename = `EnteredQuotationReqReport-All-${String(selectedMonth + 1).padStart(
          2,
          '0'
        )}-${selectedYear}${fileExtension}`;
      }
  ...
  const getModalMessage = () => {
    ...
    if (selectedReqNo && locationId) {
      return `Are you sure you want to download the Quotation Request report for Quotation No. ${selectedReqNo} at Location ${locationId} for ${period}?`;
    } else {
      return `Are you sure you want to download the Quotation Request report for all quotations for ${period}?`;
    }
  };
  ...
  return (
    <div className="max-h-screen w-full bg-gray-50 dark:bg-gray-900 flex flex-col p-2 sm:p-4 transition-all duration-300">
      ...
      <CommonHeader
        ...
        icon={<FileOutput className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8" />}
      />
      ...
      <div className="bg-white dark:bg-gray-800 p-2 sm:p-4 md:p-6 lg:p-8 rounded-2xl shadow-md w-full max-w-6xl mx-auto flex-1">
        ...
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-center gap-3 sm:gap-4 md:gap-6 mb-6 sm:mb-8">
          ...
          {/* Quotation No Dropdown with Search */}
          <div className="flex flex-col w-full lg:w-1/3 relative" ref={locationRef}>
            <div className="relative mt-2">
              <input
                ...
                type="text"
                placeholder=" "
                value={loadingDropdown ? 'Loading...' : selectedReqNo || ''}
                readOnly
                onClick={() => !loadingDropdown && setDropdownOpen(!dropdownOpen)}
            ...
            {dropdownOpen && !loadingDropdown && (
              <div className="absolute top-full mt-2 w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-xl z-20 overflow-hidden">
                <div className="p-3 border-b border-gray-200 dark:border-gray-700">
                  <input
                    type="text"
                    placeholder="Search quotation numbers..."
                    autoFocus
                    value={searchTerm}
          ...
          <div className="relative w-full lg:w-1/3 mt-2">
            <input
              id="locationId"
              type="text"
              placeholder=" "
              value={locationId}
              readOnly
              className="peer w-full px-4 py-2 border border-gray-300 dark:border-gray-600
               rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white transition-colors duration-200"
            />
            <label
              htmlFor="locationId"
              className="absolute left-4 flex gap-2 items-center top-2 text-gray-500 dark:text-gray-400 transition-all duration-200 pointer-events-none text-sm
                           peer-focus:-top-2 peer-focus:-translate-y-0.5 peer-focus:left-3 peer-focus:text-xs peer-focus:bg-white dark:peer-focus:bg-gray-700 peer-focus:px-1 peer-focus:text-blue-600
                           peer-[:not(:placeholder-shown)]:-top-2 peer-[:not(:placeholder-shown)]:-translate-y-0.5 peer-[:not(:placeholder-shown)]:left-3 
        ...
        {/* Action Buttons */}
        <div className="flex justify-center gap-3 sm:gap-4 md:gap-6 pt-4 border-gray-200 dark:border-gray-700">
         
         <Tooltip content='Download'>
          <button
            type="button"
            onClick={() => setShowConfirmModal(true)}
            disabled={loading || loadingDropdown || (selectedReqNo && !locationId)}
            className={`bg-gradient-to-r from-blue-600 to-blue-700 dark:from-blue-700 dark:to-blue-800 hover:from-blue-700 hover:to-blue-800 dark:hover:from-blue-800 dark:hover:to-blue-900 
              text-white text-base sm:text-lg p-2 sm:p-3 rounded-full shadow-lg transition-all duration-300 
              flex items-center justify-center hover:shadow-xl transform hover:scale-105 active:scale-95 
              ${
                loading || loadingDropdown || (selectedReqNo && !locationId)
...
<Tooltip content='Clear'>
        ...
        {/* Loading indicator (inline) – kept for backward compatibility */}
        {loading && (
...
export default EnteredQuatationRequest;