...
const SupplierPriceConfirmation: React.FC = () => {
  ...
  const [loading, setLoading] = useState<boolean>(false); // download loading
  ...
  // Global loading state for all API calls and initial render
  const isLoading = loadingLocations || loading;
  ...
  const fetchQtnRequests = async () => {
    try {
      ...
      const response = await fetch(
        `http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/pythonReportController/dropDownQtnRequestNoOfQtnReqHead/${formattedDate}`,
        {
          method: 'GET',
          headers: {
      ...
      console.log(`api url of dropdwon ${response.url}`);
      console.log('Response dropdwon api :', response);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to load quotations';
  ...
  const downloadFile = async () => {
    ...
    try {
      ...
      if (!consolidationId) {
        throw new Error('Please select a Quotation No');
      }

      const apiUrl = `http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/pythonReportController/supplierPriceConfirmation/${formattedDate}/${consolidationId}`;
        ...
        if (!result.success && result.message) {
          ...
          const noDataMessage =
            result.message.includes('no record') || result.message.includes('No records')
              ? 'No records found for the selected period and quotation.'
              : result.message;
      ...
      let filename = `SupplierPriceConfirmation-${consolidationId}-${formattedDate}${fileExtension}`;
  ...
  const getModalMessage = () => {
    ...
    const consolidationId = getSelectedLocationId();
    return `Are you sure you want to download the Supplier Price Confirmation report for Consolidation ${consolidationId} for ${period}?`;
  };
  ...
  return (
    <div className="max-h-screen w-full bg-gray-50 dark:bg-gray-900 flex flex-col p-2 sm:p-4 transition-all duration-300">
      ...
      {/* Header */}
      <CommonHeader
        title="Supplier Price Confirmation Report"
        icon={<PackageCheck className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8" />}
      />
          ...
          <div className="flex flex-col w-full lg:w-1/3 relative" ref={locationRef}>
            <div className="relative mt-2">
              <input
                ...
                type="text"
                placeholder=" "
                value={loadingLocations ? 'Loading quotations...' : selectedLocation || ''}
                readOnly
...
export default SupplierPriceConfirmation;