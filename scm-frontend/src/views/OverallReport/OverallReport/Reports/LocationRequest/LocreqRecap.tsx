...
import { ArrowDownToLine, X, ChevronLeft, ChevronRight, CalendarDays } from 'lucide-react';
import SessionModal from 'src/views/SessionModal';
import Toastify,{ showToast } from "src/views/Toastify";
import CommonHeader from '../../CommonHeader';
import { Tooltip} from "flowbite-react";
const LocationRequestRecap: React.FC = () => {
  ...
  const downloadFile = async () => {
    ...
    try {
      ...
      const fetchUrl = `http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/scmReportsController/listOfLocationRequestParticularReportDataPeriodBased/${formattedDate}/${userId}`;
      console.log(fetchUrl);

      const apiUrl = `http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/scmReportsController/listOfLocationRequestParticularReportDataPeriodBased/${formattedDate}/${userId}`;
      ...
      let filename = `LocationRequestRecap-${formattedDate}${fileExtension}`;
  ...
  const getModalMessage = () => {
    ...
    return `Are you sure you want to download the Location Request recap report for all locations for ${period}?`;
  };
  ...
  return (
    <div className="max-h-screen w-full bg-gray-50 dark:bg-gray-900 flex flex-col p-2 sm:p-4 transition-all duration-300">
      ...
      {/* Header */}
      <CommonHeader
      title="Location Request Recap Report"
              icon={<CalendarDays className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8" />}/>
...
export default LocationRequestRecap;
