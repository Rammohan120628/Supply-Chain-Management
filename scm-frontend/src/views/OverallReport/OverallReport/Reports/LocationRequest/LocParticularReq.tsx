...
import { MapPinCheck, ArrowDownToLine, X, ChevronLeft, ChevronRight, CalendarDays } from "lucide-react";
...
const LocationParticularRequest: React.FC = () => {
  ...
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        ...
        const response = await fetch('http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/scmReportsController/dropDownLocation', {
          method: 'GET',
          headers: {
        ...
        if (result.success && result.data) {
          ...
          if(result?.data?.length == 0 ){
            showToast('No Location Data for selected period','error')
          }
        } else {
  ...
  const filteredLocations = locations.filter((location) =>
    `${location.locationId} ${location.locationName}`.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  // Function to extract location ID from selected location string
  const extractLocationId = (locationDisplay: string): string => {
    const match = locationDisplay.match(/^[A-Z0-9]+/);
  ...
  const getSelectedLocationId = (): string => {
    ...
    // Find the actual location object to get the correct locationId
    const selectedLocationObj = locations.find(loc => 
      formatLocationDisplay(loc) === selectedLocation
    );
    
    return selectedLocationObj ? selectedLocationObj.locationId : extractLocationId(selectedLocation);
  };
  ...
  const downloadFile = async () => {
    ...
    try {
      ...
      // Determine which API to use based on location selection
      if (selectedLocation) {
        const locationId = getSelectedLocationId();
        if (!locationId) {
          throw new Error('Invalid location format');
        }
        apiUrl = `http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/scmReportsController/listOfLocationRequestParticularReportDataLocationIdBased/${formattedDate}/${locationId}/${userId}`;
      } else {
        apiUrl = `http://43.254.31.234:9070/api-gateway-scm/reports-service-scm/scmReportsController/listOfLocationRequestParticularReportDataPeriodBased/${formattedDate}/${userId}`;
      }
      ...
      let filename = selectedLocation 
        ? `LocationRequestParticular-${getSelectedLocationId()}-${formattedDate}${fileExtension}`
        : `LocationRequestParticular-AllLocations-${formattedDate}${fileExtension}`;
  ...
  const getModalMessage = () => {
    ...
    if (selectedLocation) {
      const locationName = selectedLocation.split(' ').slice(1).join(' ');
      return `Are you sure you want to download the Location Request report for ${locationName} for ${period}?`;
    } else {
      return `Are you sure you want to download the Location Request report for all locations for ${period}?`;
    }
  };

  return (
    <div className="max-h-screen w-full bg-gray-50 dark:bg-gray-900 flex flex-col p-2 sm:p-4 
        ...
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-center gap-3 sm:gap-4 md:gap-6 mb-6 sm:mb-8">
          ...
          <div className="flex flex-col w-full lg:w-1/3 relative" ref={locationRef}>
            <div className="relative mt-2">
              ...
              <span className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-500 dark:text-gray-400 transform transition-transform duration-200">▾</span>
            </div>
                ...
                <ul className="max-h-48 sm:max-h-60 overflow-y-auto">
                  {filteredLocations.length > 0 ? (
                    filteredLocations.map((location) => (
                      ...
                      >
                        <div className="font-medium text-gray-800 dark:text-gray-200 text-sm sm:text-base">{location.locationId}</div>
                        <div className="text-gray-600 dark:text-gray-400 text-xs sm:text-sm truncate">{location.locationName}</div>
                      </li>
                    ))
                  ) : (
...
export default LocationParticularRequest;