...
// ---------- Floating Input Component with inline character counter ----------
...
const ConfirmModal: React.FC<ConfirmModalProps> = ({
  ...
  const message = isStatus
    ? `Are you sure you want to change the status of "${targetName || 'this item quantity'}"?`
    : `Are you sure you want to save this new item quantity "${targetName || ''}"?`;
...
const ItemQuantityList: React.FC = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<ItemQuantityData | null>(null);
  ...
  const [newItemQuantityName, setNewItemQuantityName] = useState('');
  ...
  const columnHelper = createColumnHelper<ItemQuantityData>();

  // ----- API endpoints -----
  const BASE_URL = 'http://43.254.31.234:9070/api-gateway-scm/masters-service-scm/itemSubMasterController';
  const LIST_API_URL = `${BASE_URL}/listMstItemQuantity`;
  const VIEW_API_URL = `${BASE_URL}/viewMstItemQuantity`;
  const MODIFY_API_URL = `${BASE_URL}/modifyMstItemQuantity`;
  const STATUS_UPDATE_API_URL = `${BASE_URL}/mstItemQuantityStatusUpdate`;
  const SAVE_API_URL = `${BASE_URL}/saveMstItemQuantity`;
  ...
  const openEdit = async (item: ItemQuantityData) => {
    ...
    try {
      ...
      setEditQuantityName(fullItem.itemQuantityName || item.itemQuantityName);
      ...
      setShowSessionExpired(true);
      console.error('Error fetching item quantity details:', error);
      showToast('Error fetching item quantity details.', 'error');
      setEditQuantityName(item.itemQuantityName);
  ...
  const handleEditSave = async () => {
    ...
    const updateBody = {
      itemQuantityPk: selectedItem.id,
      itemQuantityName: trimmed,
      status: statusCode,
      ...
      setData((prevData) =>
        prevData.map((item) =>
          item.id === selectedItem.id
            ? { ...item, itemQuantityName: trimmed, status: selectedItem.status, statusColor: newStatusColor }
            : item
        )
      );
  ...
  const validateCreateFields = (): boolean => {
    const trimmed = newItemQuantityName.trim();
    if (!trimmed) {
      showToast('Item Quantity Name is required.', 'error');
      setNewErrors({ itemQuantityName: 'Item Quantity Name is required.' });
      return false;
    }
    if (trimmed.length < 4 || trimmed.length > 15) {
      showToast('Item Quantity Name must be between 4 and 15 characters.', 'error');
      setNewErrors({ itemQuantityName: 'Must be 4-15 characters.' });
      return false;
    }
    return true;
  };
...
const handleCreateSave = async () => {
  ...
  try {
    ...
    if (data.success === false) {
      if (data.message?.toLowerCase().includes('duplicate')) {
        showToast('Item quantity name already exists', 'error');
      } else {
        showToast(data.message || 'Error saving item quantity.', 'error');
      }
      return;
    }

    showToast('Item quantity saved successfully.', 'success');
    closeCreateModal();
    fetchData(); // refresh the list
  } catch (error) {
    console.error('Error saving item quantity:', error);
    showToast(
      error instanceof Error ? error.message : 'Failed to save item quantity.',
      'error',
    );
  } finally {
    NProgress.done();
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const mappedData = res.data.map((item: any) => ({
        ...
        itemQuantityName: item.itemQuantityName,
      ...
      showToast('Error fetching item quantities.', 'error');
    } finally {
      setIsLoading(false);
      NProgress.done();
    }
  ...
  const defaultColumns = [
    ...
    columnHelper.accessor('itemQuantityName', {
      header: ({ column }) => (
        <div
          className="flex items-center gap-1 cursor-pointer hover:text-blue-200"
          onClick={column.getToggleSortingHandler()}
        >
          <span className="font-medium text-white text-[10px] uppercase">Item Quantity Name</span>
          {column.getCanSort() && (
        ...
        return (
          <label className="inline-flex items-center cursor-pointer">
            <input
              ...
              checked={isActive}
              onChange={() => handleStatusChange(row.id, status, row.itemQuantityName)}
              disabled={isToggling}
            />
            <div
              className={`
    ...
    columnHelper.display({
      ...
      cell: ({ row }) => (
        <button
          ...
          aria-label={`Edit ${row.original.itemQuantityName}`}
        >
          <SquarePen size={16} />
        </button>
      ),
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-2 md:p-3">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm mb-4 mt-2 p-4">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
                ...
                content={
                  <div className="text-xs max-w-xs">
                    ...
                    <ol className="list-decimal list-inside space-y-1">
                      <li>Click the "+" button to add a new item quantity</li>
                      <li>Fill in the name (min 4, max 15 characters)</li>
                      <li>Use the modify icon to edit an existing quantity</li>
            ...
            <div className="flex gap-2">
              <Tooltip content="Add new item quantity" placement="bottom">
                <Button
                  color="blue"
                  size="xs"
                      ...
                      <td colSpan={columns.length} className="px-1.5 py-8 text-center">
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-1">
                            {debouncedSearchTerm ? 'No matching records found' : 'No item quantities yet'}
                          </h4>
                          <p className="text-gray-500 dark:text-gray-400 text-xs mb-4 text-center max-w-sm">
                            {debouncedSearchTerm
                              ? 'Try a different search term or clear the filter.'
                              : 'Get started by creating your first item quantity.'}
                          </p>
                          {!debouncedSearchTerm && (
                            <Button
                              color="blue"
                            ...
                            >
                              <HiPlus className="w-4 h-4" /> Add Item Quantity
                            </Button>
                          )}
                        </div>
                      </td>
        ...
        <Modal show={showCreateModal} onClose={closeCreateModal} size="md" dismissible>
          <ModalHeader className="rounded-lg border-b bg-white dark:bg-gray-800 text-white p-4">
            <div className="flex items-center gap-2">
              <HiPlus className="w-4 h-4" />
              <span className="text-sm font-bold">Add Item Quantity</span>
            </div>
          </ModalHeader>
          <ModalBody className="p-4 bg-white dark:bg-gray-800">
            <div className="space-y-4">
              <FloatingInput
                id="newItemQuantityName"
                name="itemQuantityName"
                value={newItemQuantityName}
                onChange={(e) => {
                  setNewItemQuantityName(e.target.value);
                  setNewErrors(prev => { const newErrors = { ...prev }; delete newErrors.itemQuantityName; return newErrors; });
                }}
                label="Item Quantity Name"
                required
                error={newErrors.itemQuantityName}
                maxLength={15}
              />
            </div>
          </ModalBody>
        ...
        <Modal show={showEditModal} onClose={closeEdit} size="md" dismissible>
          <ModalHeader className="rounded-lg border-b bg-white dark:bg-gray-800 text-white p-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold">Edit Item Quantity</span>
            </div>
          </ModalHeader>
          <ModalBody className="p-4 bg-white dark:bg-gray-800">
            <div className="space-y-4">
              <div className="flex justify-end mb-2">
                <button
                  onClick={() => {
                    if (selectedItem) {
                      handleStatusChange(selectedItem.id, selectedItem.status, selectedItem.itemQuantityName);
                      closeEdit();
                    }
                  }}
              ...
              <FloatingInput
                id="editItemQuantityName"
                name="itemQuantityName"
                value={editQuantityName}
                onChange={(e) => {
                  setEditQuantityName(e.target.value);
                  setErrors(prev => { const newErrors = { ...prev }; delete newErrors.itemQuantityName; return newErrors; });
                }}
                label="Item Quantity Name"
                required
                error={errors.itemQuantityName}
                maxLength={15}
              />
            </div>
          </ModalBody>
