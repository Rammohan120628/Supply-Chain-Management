...
import StockViewScreen from './StockView';
...
export interface StockReceiveItem {
  ...
  cwhDelId: string;
  ordLocId: string;
  ordLocIdDate: string | null;
  ordLocIdDateStr: string;
  poNumber: string;
  deliveryNote: string;
  locationName: string;
  discount: number;
...
const StockTable = ({ onBack }) => {
  ...
  const handleViewClick = (rowData: StockReceiveItem) => {
    console.log('View clicked for CWH Del Id:', rowData.cwhDelId);
    console.log('Full row data:', rowData);
    setSelectedRow(rowData);
  };

  // Use useMemo for columns - REDESIGNED with compact styling
...
const columns = useMemo(() => [
...
},
  {
    id: 'cwhDelId',
      ...
      >
        <span className="font-medium text-white text-[10px] uppercase">CWH Del Id</span>
        {column.getCanSort() && (
  ...
  },
  {
    ...
    cell: (info: any) => (
      <span className="text-[11px] font-medium text-black break-words block max-w-[90px]">
        {info.row.original.deliveryNote || 'N/A'}
      </span>
    ),
    size: 85,
    enableSorting: true,
  },
  {
    id: 'delDateStr',
      ...
      <div
        ...
        onClick={column.getToggleSortingHandler()}
      >
        <span className="font-medium text-white text-[10px] uppercase">Del Date</span>
        {column.getCanSort() && (
          <span className="text-[10px]">
            {{
              asc: ' 🔼',
  ...
  },
  {
    ...
    accessorFn: (row) => `${row.ordLocId} ${row.locationName}`,
    cell: (info: any) => (
      <div className="min-w-[100px]">
        <div className="text-[11px] font-medium leading-tight text-black break-all">{info.row.original.ordLocId || ''}</div>
        <div className="text-[10px] leading-tight break-all">{info.row.original.locationName || ''}</div>
      </div>
    ),
    size: 110,
    enableSorting: true,
  ...
  
  {
    ...
    cell: (info: any) => (
      <div className="min-w-[100px]">
        <div className="text-[11px] font-medium leading-tight break-all text-black">{info.row.original.userId}</div>
        <div className="text-[9px] leading-tight break-all">{formatDateTime(info.row.original.createdDataTime)}</div>
      </div>
    ),
    size: 110,
    enableSorting: true,
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (searchValue.trim()) {
      ...
      setGridCurrentPage(1); // Reset grid page on search
    } else {
  ...
  const fetchStockReceiveData = async (selectedPeriod: string) => {
    ...
    try {
      const apiUrl = `http://43.254.31.234:9070/api-gateway-scm/stock-movement-analysis-scm/stockDeliveryController/cwhLocationList/${selectedPeriod}`;
  ...
  const exportToExcel = async () => {
    ...
    const excelApiUrl = `http://43.254.31.234:9070/api-gateway-scm/stock-movement-analysis-scm/stockDeliveryController/downloadDeliveryItemToLocation/${period}`;
    ...
    try {
      ...
      let filename = `Delivery_Item_To_Location_List.xlsx`;
  ...
  // If selectedRow exists, return StockViewScreen
  if (selectedRow) {
    console.log('Rendering StockViewScreen for CWH Del Id:', selectedRow.cwhDelId);
    return (
      <StockViewScreen
        rowData={selectedRow}
        onBack={() => {
  ...
  const DashboardCards = () => {
    ...
    const recentCount = data.filter(item => {
      if (!item.createdDataTime) return false;
      const createdDate = new Date(item.createdDataTime);
    ...
    const SmallCard = [
      ...
      },
      {
        ...
        bgcolor: "error",
        colorClass: "error",
        desc: "Deliveries in last 30 days",
      },
      {
        icon: "mdi:history",
        num: olderCount,
        title: "Older Deliveries",
        shape: shape2,
        bgcolor: "secondary",
        colorClass: "secondary",
        desc: "Deliveries older than 30 days",
      },
    ];
  ...
  const DeliveryItemGrid = () => {
    const getStatusBadge = (status: number | undefined) => {
      if (status === 0) {
        return { color: 'blue', text: 'Pending' };
      } else if (status === 1) {
        return { color: 'warning', text: 'Delivered' };
      }
      ...
      <>
        ...
        <div className="relative grid-container-scroll max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-3 sm:gap-5">
            {currentGridItems.map((item, index) => {
              ...
              return (
                <CardBox 
                  key={item.cwhDelId || index} 
                  className="hover:shadow-md transition-shadow duration-300 border border-gray-200 h-auto p-3 sm:p-4"
                >
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-gray-800 text-sm sm:text-base truncate">{item.cwhDelId || 'N/A'}</h3>
                      <p className="text-xs text-black mt-0.5 truncate">{item.deliveryNote || 'No Del Note'}</p>
                      <p className="text-xs text-black mt-0.5 font-bold truncate">
                        <span className='text-blue-600'>Date: </span>{item.delDateStr || 'N/A'}
                      </p>
                    </div>
                  ...
                  <div className="mb-2">
                    ...
                    <p className="text-xs font-semibold text-gray-800 break-words">{item.ordLocId || 'N/A'}</p>
                    <p className="text-xs text-gray-600 break-words line-clamp-2">{item.locationName || 'N/A'}</p>
                  </div>
                  
                  <div className="bg-blue-50 p-2 sm:p-3 rounded-md mt-2">
                    <div className="flex justify-between items-center">
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-black mb-0.5">Supplier</p>
        ...
        {filteredData.length === 0 && (
          <div className="mt-4 p-4 bg-gray-50 rounded-lg text-center">
            ...
            <p className="text-sm text-gray-600">No records found for this period</p>
            {globalFilter && (
              <p className="text-xs text-gray-500 mt-1">No results for "{globalFilter}"</p>
            )}
          </div>
        )}
  ...
  return (
    ...
    <div className="w-full max-w-full mx-auto px-2 sm:px-2 ">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb- sm:mb-">
        <h1 className="text-lg sm:text-xl lg:text-xl text-indigo-700 whitespace-normal break-words">
          Delivery Item To Location
        </h1>
        
        <div className="flex bg-gray-100 rounded-lg p-1 w-full sm:w-auto mt- gap-2">
          <button
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 mb-4">
            ...
            <div className="w-full sm:flex-1 relative" ref={periodRef}>
              <div className="relative">
                ...
                <CalendarDays className="absolute right-128 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4 sm:w-5 sm:h-5 pointer-events-none" />
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
              <input
                ...
                className="max-w-80 sm:w-auto sm:flex-1 px-3 py-2 text-xs sm:text-sm border border-gray-300 rounded-md focus:outline-none focus:border-blue-500"
                value={globalFilter}
