...
const modalColumnHelper = createColumnHelper<ItemModalType>();
    ...
    cell: (info) => (
      <div>
        <div className="flex flex-col gap-1">
          <h6 className="text-[11px] font-bold text-black dark:text-white">
            {info.getValue()}
          </h6>
        </div>
      </div>
  ...
  columnHelper.accessor('createdDate', {
    ...
    header: ({ column }) => (
      <div
        ...
        onClick={column.getToggleSortingHandler()}
      >
        <span className="font-medium text-white text-[10px] uppercase">Created Date</span>
        {column.getCanSort() && (
          <span className="text-[10px]">
            {{
              asc: ' 🔼',
    ...
    cell: (info) => (
      <div>
        <div className="flex items-center gap-1">
          <span className="text-[11px] font-bold text-black dark:text-white">{info.getValue()}</span>
        </div>
      </div>
    ),
  }),
...
const defaultColumns2Full = (formatter: ReturnType<typeof useEntityFormatter>) => [
  modalColumnHelper.accessor('itemId', {
    enableSorting: true,
    header: ({ column }) => (
      <div
        className="flex items-center gap-1 cursor-pointer hover:text-blue-200"
  ...
  modalColumnHelper.accessor('itemName', {
    enableSorting: true,
    header: ({ column }) => (
      <div
        className="flex items-center gap-1 cursor-pointer hover:text-blue-200"
  ...
  modalColumnHelper.accessor('supplierId', {
    enableSorting: true,
    header: ({ column }) => (
      <div
        className="flex items-center gap-1 cursor-pointer hover:text-blue-200"
  ...
  modalColumnHelper.accessor('supplierName', {
    enableSorting: true,
    header: ({ column }) => (
      <div
        className="flex items-center gap-1 cursor-pointer hover:text-blue-200"
  ...
  modalColumnHelper.accessor('grandTotal', {
    enableSorting: true,
    header: ({ column }) => (
      <div
        className="flex items-center gap-1 cursor-pointer hover:text-blue-200"
  ...
  modalColumnHelper.accessor('gross', {
    enableSorting: true,
    header: ({ column }) => (
      <div
        className="flex items-center gap-1 cursor-pointer hover:text-blue-200"
  ...
  modalColumnHelper.accessor('net', {
    enableSorting: true,
    header: ({ column }) => (
      <div
        className="flex items-center gap-1 cursor-pointer hover:text-blue-200"
...
const defaultColumns2Reduced = (formatter: ReturnType<typeof useEntityFormatter>) => {
  ...
  return [full[0], full[1], full[4]]; // Item ID, Item Name, Quantity
};

interface ConsolidateQuotationReqTableProps {
  onBack?: () => void;
}

const ConsolidateQuotationReqTable = ({ onBack }: ConsolidateQuotationReqTableProps) => {
  ...
  // Data states
  const [consolidatedList, setConsolidatedList] = useState<TableTypeDense[]>([]);
  const [quotationList, setQuotationList] = useState<TableTypeDense[]>([]);
  const [finalizedList, setFinalizedList] = useState<TableTypeDense[]>([]);
  ...
  const [modalData, setModalData] = useState<ItemModalType[]>([]);
  const [modalSearch, setModalSearch] = useState('');
  const [modalLoading, setModalLoading] = useState(false);
  const [viewSourceTab, setViewSourceTab] = useState<number | null>(null); // 0=consolidated,1=quotation,2=finalized
  ...
  // Min/max year/month from tender period
  const [minYear, setMinYear] = useState(new Date().getFullYear() - 2);
  const [maxYear, setMaxYear] = useState(new Date().getFullYear());
  const [minMonth, setMinMonth] = useState(0);
  const [maxMonth, setMaxMonth] = useState(11);

  // ----------------------------------------------------------------------
  // Effect: initialise from tenderPeriod
  // ----------------------------------------------------------------------
  useEffect(() => {
    const parts = tenderPeriod ? tenderPeriod.split('-') : [];
    if (parts.length !== 3) {
    ...
    setMinYear(twoYearsAgo.getFullYear());
    setMaxYear(date.getFullYear());
    setMinMonth(twoYearsAgo.getMonth());
    setMaxMonth(date.getMonth());
    const initialPeriod = formatDateForApi(date.getMonth(), date.getFullYear());
  ...
  // Data mapping
  // ----------------------------------------------------------------------
  const mapApiDataToTable = (apiData: any[]): TableTypeDense[] => {
    return apiData.map((item) => ({
      consolidationId: item.consolidationId || 'N/A',
      period: item.periodStr || 'N/A',
      createdDate: item.lastUpdate ? item.lastUpdate.split('T')[0] : 'N/A',
    }));
  };

  const mapModalData = (apiData: any[]): ItemModalType[] => {
    return apiData.map((item) => ({
      itemId: item.itemId?.toString() || 'N/A',
      itemName: item.itemName || 'N/A',
      packageId: item.packageId || 'N/A',
      supplierId: item.supplierId || 'N/A',
      supplierName: item.supplierName || 'N/A',
      gross: item.gp ? formatter.formatAmount(item.gp) : formatter.formatAmount(0),
      net: item.netValue ? formatter.formatAmount(item.netValue) : formatter.formatAmount(0),
      grandTotal: item.grandTotal ? formatter.formatAmount(item.grandTotal) : formatter.formatAmount(0),
      requestDate: item.requestDateStr || item.requestDate || 'N/A',
    }));
  };
  ...
  const fetchData = async (period?: string) => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://43.254.31.234:9070/api-gateway-scm/tender-process-service-scm/tenderProcessController/consolidationDetailsListAPI/${finalPeriod}`,
        {
          headers: {
      ...
      const { data } = response;
      if (data.success) {
        setConsolidatedList(mapApiDataToTable(data.data.consolidation || []));
        setQuotationList(mapApiDataToTable(data.data.quotationReply || []));
        setFinalizedList(mapApiDataToTable(data.data.finalizetheSupplierSelection || []));
      } else {
  ...
  // Fetch modal data
  // ----------------------------------------------------------------------
  const fetchModalData = async (consId: string) => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://43.254.31.234:9070/api-gateway-scm/tender-process-service-scm/tenderProcessController/consolidationSubList/${consId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
      ...
      const { data } = response;
      if (data.success) {
        setModalData(mapModalData(data.data || []));
      } else {
  ...
  const handleViewClick = (rowData: TableTypeDense, sourceTab?: number) => {
    if (rowData.consolidationId && rowData.consolidationId !== 'N/A') {
      const tabToUse = sourceTab !== undefined ? sourceTab : activeTab;
      setViewSourceTab(tabToUse);
      setSelectedId(rowData.consolidationId);
      setModalData([]);
      setModalSearch('');
      setModalSorting([]);
      fetchModalData(rowData.consolidationId);
      setOpenModal(true);
    } else {
      showToast('Invalid consolidation ID', 'error');
    }
  };
  ...
  const isPeriodDisabled = (index: number) => {
    if (selectedYear === minYear && index < minMonth) return true;
    if (selectedYear === maxYear && index > maxMonth) return true;
    return false;
  };
  ...
  const filteredConsolidatedList = useMemo(
    () =>
      consolidatedList.filter((row) =>
        Object.values(row).some((val) =>
          val?.toString().toLowerCase().includes(globalSearch.toLowerCase())
        )
      ),
    [consolidatedList, globalSearch]
  );

  const filteredQuotationList = useMemo(
    () =>
      quotationList.filter((row) =>
        Object.values(row).some((val) =>
          val?.toString().toLowerCase().includes(globalSearch.toLowerCase())
        )
      ),
    [quotationList, globalSearch]
  );

  const filteredFinalizedList = useMemo(
    () =>
      finalizedList.filter((row) =>
        Object.values(row).some((val) =>
          val?.toString().toLowerCase().includes(globalSearch.toLowerCase())
        )
      ),
    [finalizedList, globalSearch]
  );

  const filteredModalData = useMemo(
    () =>
      modalData.filter((row) =>
        Object.values(row).some((val) =>
          val?.toString().toLowerCase().includes(modalSearch.toLowerCase())
        )
      ),
  ...
  // ----------------------------------------------------------------------
  // Column definitions (using formatter)
  // ----------------------------------------------------------------------
  ...
  const modalColumns = useMemo(() => {
    const base = viewSourceTab === 2 ? defaultColumns2Full(formatter) : defaultColumns2Reduced(formatter);
    return [
      modalColumnHelper.display({
        id: 'sno',
        header: () => <span className="font-medium text-white text-[10px] uppercase">S.No</span>,
        cell: ({ row }) => <span className="text-[11px]">{modalStartIndex + row.index + 1}</span>,
        size: 40,
  ...
  }, [formatter, viewSourceTab, modalStartIndex]);

  // Determine which data to show based on active tab
  const getActiveData = () => {
    switch (activeTab) {
      case 0:
        return filteredConsolidatedList;
      case 1:
        return filteredQuotationList;
      case 2:
        return filteredFinalizedList;
      default:
        return [];
    }
  };
  ...
  const getModalTitle = () => {
    switch (viewSourceTab) {
      case 0:
        return 'Item Details - Consolidated Items';
      case 1:
        return 'Item Details - Quotation Prepared Items';
      case 2:
        return 'Item Details - Finalized Items';
      default:
        return 'Item Details';
    }
  };
    ...
    <div className="w-[1050px]">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md mb-3 text-sm p-2">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <h1 className="text-lg sm:text-xl lg:text-xl text-indigo-700 whitespace-nowrap">
            Consolidate Quotation Request List
          </h1>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md p-2 sm:p-4">
        <div className="w-full">
            ...
            <div className="w-full sm:flex-1 relative" ref={periodRef}>
              <div className="relative">
                ...
                <label className="absolute left-3 sm:left-4 top-2 text-xs sm:text-sm text-gray-600 dark:text-gray-300 transition-all duration-200 pointer-events-none peer-focus:-top-2.5 peer-focus:left-2 peer-focus:text-[10px] sm:peer-focus:text-xs peer-focus:bg-white dark:peer-focus:bg-gray-800 peer-focus:px-1 peer-focus:text-blue-600 peer-[:not(:placeholder-shown)]:-top-2.5 peer-[:not(:placeholder-shown)]:left-2 peer-[:not(:placeholder-shown)]:text-[10px] sm:peer-[:not(:placeholder-shown)]:text-xs peer-[:not(:placeholder-shown)]:bg-white dark:peer-[:not(:placeholder-shown)]:bg-gray-800 peer-[:not(:placeholder-shown)]:px-1">
                  Tender Period <sup className="text-red-600">*</sup>
                </label>
          ...
          {/* Tabs */}
          <Tabs
            aria-label="Consolidate quotation tabs"
            variant="underline"
            ...
            <TabItem
              ...
              title={
                <div className="flex items-center gap-1">
                  ...
                  <span className="dark:text-white">Quotation Prepared</span>
                  <Badge color="warning" className="text-[10px] dark:bg-yellow-900 dark:text-yellow-100 px-1.5 py-0.5">
                    {filteredQuotationList.length}
                  </Badge>
                </div>
              }
            >
            ...
            <TabItem
              ...
              title={
                <div className="flex items-center gap-1">
                  ...
                  <span className="dark:text-white">Finalized</span>
                  <Badge color="success" className="text-[10px] dark:bg-green-900 dark:text-green-100 px-1.5 py-0.5">
                    {filteredFinalizedList.length}
                  </Badge>
                </div>
              }
            >
      ...
      <Modal show={openModal} onClose={() => setOpenModal(false)} size="4xl">
        <ModalHeader className="border-b border-gray-200 dark:border-gray-700 p-2 bg-white dark:bg-gray-800 rounded-md">
          <div className="flex items-center justify-between w-full">
            ...
            {/* Modal search input - fixed top right without extra margin */}
            <div className="relative w-56 p-1 ml-70">
        ...
        <ModalBody className="p-7 bg-white dark:bg-gray-800 rounded-md">
                        ...
                        {row.getVisibleCells().map((cell) => (
                          <td
                            ...
                            className={`px-2 py-1.5 text-xs text-gray-800 dark:text-gray-300 pr-14 ${
                              ['grandTotal', 'gross', 'net'].includes(cell.column.id) ? 'text-right' : ''
                            }`}
                          >
...
export default ConsolidateQuotationReqTable;