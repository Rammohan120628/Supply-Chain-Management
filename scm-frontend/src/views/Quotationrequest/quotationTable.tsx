...
// Main table columns with sorting (base without serial number)
const defaultColumns = (formatter: ReturnType<typeof useEntityFormatter>) => [
  ...
  columnHelper.accessor('status', {
    ...
    cell: (info) => (
      <p className="text-[11px] font-bold dark:text-gray-300 text-black capitalize dark:bg-opacity-20">
        {info.getValue()}
      </p>
    ),
  }),
...
const defaultColumns1 = (formatter: ReturnType<typeof useEntityFormatter>) => {
...
const QuotationReqTable = ({ onBack }: { onBack?: () => void }) => {
  ...
  const [consolidatedListData, setConsolidatedListData] = useState<TableTypeDense[]>([]);
  const [quotationReplyListData, setQuotationReplyListData] = useState<TableTypeDense[]>([]);
  const [finalizedListData, setFinalizedListData] = useState<TableTypeDense[]>([]);
  ...
  const mapApiDataToTable = (apiData: any[], hasConsolidation: boolean): TableTypeDense[] => {
    return apiData.map((item) => ({
      ...
      statuscolor: hasConsolidation ? 'success' : 'warning',
      budget: item.grandTotal || '0',
    }));
  };
  ...
  // Initialize with tenderPeriod from localStorage
  useEffect(() => {
  ...
  // Enhanced fetchData with token check and session expiry handling
  const fetchData = async (period?: string) => {
    setIsLoading(true);
    setCreateListData([]);
    setConsolidatedListData([]);
    setQuotationReplyListData([]);
    setFinalizedListData([]);
    ...
    try {
      ...
      if (!token) {
        ...
        return; // will go to finally and stop loading
      }

      const finalPeriod = period || formatDateForApi(selectedMonth, selectedYear);
      const response = await fetch(
        `http://43.254.31.234:9070/api-gateway-scm/tender-process-service-scm/tenderProcessController/quotationRequestHeaderList/${finalPeriod}`,
        {
          method: 'GET',
          headers: {
      ...
      if (result.success) {
        setCreateListData(mapApiDataToTable(result.data.locReqCreationList || [], false));
        setConsolidatedListData(mapApiDataToTable(result.data.consolidationList || [], true));
        setQuotationReplyListData(mapApiDataToTable(result.data.quotationReplyList || [], true));
        setFinalizedListData(mapApiDataToTable(result.data.finalizetheSupplierSelectionList || [], true));
      } else {
      ...
      // Optionally handle network errors – we don't trigger session expired here
    } finally {
      setIsLoading(false);
    }
  };

  // Enhanced fetchItemDetails with token check and session expiry handling
  const fetchItemDetails = async (pk: number) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://43.254.31.234:9070/api-gateway-scm/tender-process-service-scm/tenderProcessController/quotationRequestSubList/${pk}`,
        {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleViewClick = (rowData: TableTypeDense, sourceTab?: number) => {
    if (rowData.locationRequestHeaderPk) {
      const tabToUse = sourceTab !== undefined ? sourceTab : activeTab;
      setViewSourceTab(tabToUse);
      fetchItemDetails(rowData.locationRequestHeaderPk);
      setOpenModal(true);
    }
  };
  ...
  const filteredConsolidatedList = useMemo(
    () =>
      consolidatedListData.filter((row) =>
        Object.values(row).some((val) => val?.toString().toLowerCase().includes(globalSearch.toLowerCase()))
      ),
    [consolidatedListData, globalSearch]
  );
  const filteredQuotationReplyList = useMemo(
    () =>
      quotationReplyListData.filter((row) =>
        Object.values(row).some((val) => val?.toString().toLowerCase().includes(globalSearch.toLowerCase()))
      ),
    [quotationReplyListData, globalSearch]
  );
  const filteredFinalizedList = useMemo(
    () =>
      finalizedListData.filter((row) =>
        Object.values(row).some((val) => val?.toString().toLowerCase().includes(globalSearch.toLowerCase()))
      ),
    [finalizedListData, globalSearch]
  );
  ...
  // Main table columns with serial number and view button
  ...
  const columns1 = useMemo(
    () => {
      const base = defaultColumns1(formatter);
      const withView = [
        ...base,
        columnHelper.display({
          id: 'view',
        ...
        columnHelper.display({
          ...
          cell: ({ row }) => (
            <Tooltip content="View item details" className="dark:bg-gray-800 dark:text-white">
              <button
                className="text-blue-600 hover:text-blue-800 text-[11px] px-1 py-0.5 flex items-center gap-1 bg-blue-50 hover:bg-blue-100 rounded transition-colors"
                onClick={() => handleViewClick(row.original, 2)}
                title="View Details"
              >
                <Eye className="w-3.5 h-3.5" />
              </button>
        ...
        columnHelper.display({
          ...
          cell: ({ row }) => (
            <Tooltip content="View item details" className="dark:bg-gray-800 dark:text-white">
              <button
                className="text-blue-600 hover:text-blue-800 text-[11px] px-1 py-0.5 flex items-center gap-1 bg-blue-50 hover:bg-blue-100 rounded transition-colors"
                onClick={() => handleViewClick(row.original, 3)}
                title="View Details"
              >
                <Eye className="w-3.5 h-3.5" />
              </button>
  ...
  const getActiveData = () => {
    switch (activeTab) {
      ...
      case 2:
        return filteredQuotationReplyList;
      case 3:
  ...
  // Modal columns with serial number
  const modalColumns = useMemo(() => {
    const base = viewSourceTab === 3 ? defaultColumns2Full(formatter) : defaultColumns2Reduced(formatter);
    return [
      columnHelper.display({
        id: 'sno',
        header: () => <span className="font-medium text-white text-[10px] uppercase">S.No</span>,
  ...
  const getModalTitle = () => {
    switch (viewSourceTab) {
      case 0:
        return 'Item Details - Created Items';
      case 1:
  ...
  const totalRequests =
    ...
    consolidatedListData.length +
    quotationReplyListData.length +
    finalizedListData.length;
  const quotationPreparedCount = quotationReplyListData.length;
  const finalizedCount = finalizedListData.length;
    ...
    <div className="w-[1050px]">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md p-2 sm:p-4">
        <div className="w-full">
          ...
          {/* Tabs */}
          <Tabs
            aria-label="Quotation request tabs"
            variant="underline"
              ...
              title={
                <div className="flex items-center gap-1">
                  ...
                  <Badge color="warning" className="text-[10px] dark:bg-yellow-900 dark:text-yellow-100 px-1.5 py-0.5">
                    {filteredQuotationReplyList.length}
                  </Badge>
                </div>
              }
            >
              ...
              <table className="min-w-full table-fixed">
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {table2.getRowModel().rows.length > 0 ? (
                    ...
                    <tr>
                      <td
                        colSpan={table2.getAllColumns().length}
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id} className={headerBg}>
                {headerGroup.headers.map((header) => (
                  <th
                    key={header.id}
                    className="px-1.5 py-1 text-left font-medium text-white dark:text-gray-100 uppercase text-[10px] leading-tight cursor-pointer hover:opacity-90 transition-colors dark:border-gray-600"
                    style={{
              ...
              currentRows.map((row) => (
                <tr key={row.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 even:bg-gray-50/50 dark:even:bg-gray-700/50">
                  {row.getVisibleCells().map((cell) => (
                    ...
                    >
                      <div className="leading-tight min-h-[24px] flex items-start text-[11px] text-gray-900 dark:text-gray-100">
                        {flexRender(cell.column.columnDef.cell, cell.getContext())}
                      </div>
                    </td>
                  ))}
...
export default QuotationReqTable;