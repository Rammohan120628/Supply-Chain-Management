...
import QuotationReqTable from "./quotationTable";
import SessionModal from "../SessionModal";
import { useEntityFormatter } from "../Entity/UseEntityFormater";
import { Calendar, MapPin, MapPinCheck, Pin, PinIcon, RefreshCcw, X } from "lucide-react";
...
const Quotationrequest = () => {
  ...
  const handleQuantityInputChange = useCallback(
    (itemId: number, date: string, value: string) => {
      ...
      const decimalPlaces = formatter.getQuantityDecimalPlaces
        ? formatter.getQuantityDecimalPlaces()
        : 2;
  ...
  const handleSave = async () => {
    ...
    for (let d = 1; d <= 31; d++) qtyRendered[`qtyRendered${d}`] = false;
    selectedDates.forEach((d) => (qtyRendered[`qtyRendered${d.day}`] = true));
    const payload = {
      ...
      ...qtyRendered,
      subList: selectedItems.map((it, index) => {
        const itemPayload: any = {
          itemId: it.itemId,
          packageId: it.packageId,
          entOrder: index + 1,
          supplierId: it.supplierId || "N/A",
          supplierName: it.supplierName || "N/A",
        };
        selectedDates.forEach((d) => {
          const val = parseFloat(it.quantities?.[d.date] || "0.0");
          if (val >= 0) itemPayload[`qty${d.day}`] = val;
        });
        return itemPayload;
      }),
    };
    try {
      const { data } = await axios.post(
        "http://43.254.31.234:9070/api-gateway-scm/tender-process-service-scm/tenderProcessController/saveQuotationRequest",
        payload,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const totalQuantitySum = useMemo(() => {
    return selectedItems.reduce((sum, item) => sum + parseFloat(item.totalQty || "0"), 0);
  }, [selectedItems]);

  /* ────────────────────── RENDER ────────────────────── */
  if (showTable) {
    return <QuotationReqTable onBack={() => setShowTable(false)} />;
  }
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-2 md:p-3">
        ...
        <div>
          ...
          {/* Main Card */}
<Card className="border border-gray-200 dark:border-gray-700 dark:bg-gray-800 shadow-lg hover:shadow-xl transition-shadow duration-300">            {/* Top Section: Period and Location */}
          ...
          <ModalFooter className="border-t bg-gray-50 dark:bg-gray-700 p-1">
            <div className="flex items-center justify-between w-full">
              ...
              <Button
                ...
                className="text-[10px] px-3 py-0.5 dark:bg-gray-600 dark:hover:bg-gray-500 dark:text-white transition-all duration-200 hover:scale-105"
              >
                Okay
              </Button>
            </div>
