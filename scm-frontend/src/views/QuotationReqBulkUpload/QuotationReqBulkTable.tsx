...
import QuotataionRequestBulkkUpload from './QuotationReqBulkUpload';
...
export interface TableTypeDense {
  ...
  status?: string;          // deliveryMode in main, supplierName in modal
  teams?: string;
  statuscolor?: string;
  budget?: string;           // grandTotal or quantity
  gross?: string;
  net?: string;
}
...
const defaultColumns = (formatter: ReturnType<typeof useEntityFormatter>) => [
  ...
  columnHelper.accessor('status', {
    ...
    cell: (info) => (
      <p className="text-[11px] font-bold dark:text-white text-black capitalize dark:bg-opacity-20">
        {info.getValue()}
      </p>
    ),
  }),
...
const QuotationReqBulkTable = () => {
  ...
  const [isBulkMainScreen, setIsBulkMainScreen] = useState(false);
    ...
    () => {
      ...
      return [
        columnHelper.display({
          ...
          cell: ({ row }) => <span className="text-[11px]">{row.index + 1 + startIndex}</span>,
          size: 40,
          enableSorting: false,
        }),
        ...withView,
  ...
  const modalColumns = useMemo(() => {
    ...
    return [
      columnHelper.display({
        id: 'sno',
        header: () => <span className="font-medium text-white text-[10px] uppercase">S.No</span>,
cell: ({ row }) => <span className="text-[11px]">{modalStartIndex + row.index + 1}</span>,        size: 40,
        enableSorting: false,
      }),
      ...base,
    ];
  ...
  // Card data (not used but kept for consistency)
  const totalRequests =
  ...
  // If bulk upload screen is active, render it
  if (isBulkMainScreen) {
    return (
      <QuotataionRequestBulkkUpload
        onSuccess={() => {
          setIsBulkMainScreen(false);
          handleRefreshClick();
        }}
      />
    );
            ...
            <div className="w-full sm:max-w-xs">
              <div className="relative text-gray-400 focus-within:text-gray-600">
                ...
                <input
                  ...
                  className="ml-3form-control-input w-74 pl-8 px-3 py-2 sm:py-2.5 border rounded-md text-sm"
                  value={globalSearch}
...
export default QuotationReqBulkTable;